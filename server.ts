...
export async function createApp({ startListening = false } = {}) {
  ...
  app.use((req, res, next) => {
    ...
    const isTranslate = url.startsWith("/api/translate");
    const translatePatterns = isTranslate ? ["evaluate filter", "../", "..\\"] : [];
    if (leaves.some((s) => contentPatterns.some((p) => s.includes(p)) || translatePatterns.some((p) => s.includes(p)))) {
      logThreat(req.ip || "", "Injection/Pentest Payload", req.url);
