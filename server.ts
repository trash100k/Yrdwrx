...
async function startServer() {
        ...
        for (const step of interaction.steps) {
          if (step.type === 'model_output') {
            ...
            if (imageContent && (imageContent as any).data) {
                const base64Str = (imageContent as any).data;
                const mType = (imageContent as any).mime_type || 'image/png';
          ...
          if (interaction.status === "completed") {
             ...
             for (const step of interaction.steps) {
                 if (step.type === 'model_output') {
                     ...
                     if (textContent) fullReport += (textContent as any).text;
                 }
             }
  ...
  app.post("/api/auth/magic-link/validate", (req, res) => {
    try {
      ...
      res.json({ valid: true, clientId: (decoded as any).clientId, email: (decoded as any).email });
    } catch (err: any) {
