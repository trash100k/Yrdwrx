...
import {
  ...
  manualFallback,
  type MeasureResult,
} from "./src/lib/measureAdapter.js";
...
function handleAiError(res: any, e: any, context = "AI request failed") {
  ...
  if (rl.isRateLimited) {
    const retryAfterSec =
      rl.retryAfterSec ??
      (Number(process.env.AI_RETRY_AFTER_DEFAULT_SEC) || 30);
    res.set("Retry-After", String(retryAfterSec));
    console.warn(
      context + " [rate-limited upstream]:",
      e?.status ?? "",
      e?.message || e,
    );
    return res
      .status(429)
      .json({
        error: "AI is rate-limited upstream — please retry shortly.",
    ...
    for (const group of sc)
      for (const svc of group?.services || []) {
        if (svc?.name && typeof svc.price === "number")
          out.push({ name: String(svc.name).toLowerCase(), price: svc.price });
      }
  }
  return out;
}
  ...
  for (const mat of materials) {
    ...
    if (itemName && catalog.length) {
      const hit = catalog.find(
        (c) => itemName.includes(c.name) || c.name.includes(itemName),
      );
      if (hit) {
        mat.estimatedCost = hit.price;
        mat.priceSource = "catalog";
      }
    }
...
async function renderPdf(
  html: string,
  pdfOptions: any = { format: "A4", printBackground: true },
): Promise<Buffer> {
  ...
  try {
    ...
    try {
      ...
      /* interception unsupported — JS is still disabled */
    }
    ...
    try {
      ...
      /* page died with a crashed browser */
    }
    pdfInFlight--;
    ...
    (err: any, addresses: any) => {
      ...
      if (publicOnly.length === 0) {
        return cb(
          Object.assign(
            new Error(
              "SSRF blocked: host does not resolve to a public address",
            ),
            { code: "ESSRFBLOCKED" },
          ),
        );
...
async function geocodeResolve(address: any): Promise<any | null> {
  ...
  const mapsKey =
    process.env.GOOGLE_MAPS_PLATFORM_KEY || process.env.GOOGLE_MAPS_API_KEY;
  if (!mapsKey) {
  ...
  try {
    ...
    const result = {
      lat: loc.lat,
      lng: loc.lng,
      formatted: first?.formatted_address || undefined,
      stub: false,
      source: "google",
    };
...
async function stampGeocode<T extends Record<string, any>>(row: T): Promise<T> {
  try {
    ...
    /* geocode-on-write is best-effort; never block the write */
  }
  return row;
}
...
if (isMockMode) {
  ...
  // @ts-ignore
  ai.models.generateVideos = async () => {
    throw new AiUnavailableError(
      "Video generation requires GEMINI_API_KEY",
      "VIDEO_UNAVAILABLE",
    );
  };
...
function getMockText(request: any): string {
  ...
  if (instr.includes("vendor-invoice extraction engine")) {
    return JSON.stringify({
      ...
      lineItems: [
        {
          description: "Double-Shredded Hardwood Mulch",
          amount: 240,
          quantity: 4,
        },
  ...
  if (instr.includes("contract and permit extraction engine")) {
    return JSON.stringify({
      ...
      keyTerms: [
        ...
        "Auto-renews unless cancelled 30 days prior to expiration",
      ],
      obligations: [
        "Maintain proof of liability insurance",
        "Service weekly April through October",
      ],
      permitNumber: null,
    ...
    return JSON.stringify({
      tiers: {
        good: {
          ...
          estimatedMaterials: [
            {
              item: "Double-Shredded Hardwood Mulch",
              quantity: "2 cubic yards",
              estimatedCost: 100,
            },
            {
              item: "Turf-Type Tall Fescue Seed",
              quantity: "25 lb",
              estimatedCost: 90,
            },
          ],
          totalCost: 450,
          description:
            "Budget-friendly refresh using standard materials and smaller plant stock.",
        },
        better: {
          name: "Better (Standard)",
          estimatedMaterials: [
            {
              item: "Limelight Hydrangea (3-Gallon)",
              quantity: "7 shrubs",
              estimatedCost: 315,
            },
            {
          ...
          totalCost: 850,
          description: "The recommended balance of quality and value.",
        },
        best: {
          name: "Best (Premium)",
          estimatedMaterials: [
            {
              item: "Limelight Hydrangea (7-Gallon, mature)",
              quantity: "7 shrubs",
              estimatedCost: 560,
            },
            {
              item: "Low-Voltage Landscape Lighting",
              quantity: "4 fixtures",
              estimatedCost: 600,
            },
          ],
          totalCost: 1900,
          description:
            "Premium mature plantings plus accent lighting for maximum curb appeal.",
        },
      },
    });
  }
  if (
    instr.includes("Cutty Logic Core") ||
    instr.includes("landscape architect")
  ) {
    return JSON.stringify({
      identifiedAreas: [
        {
          id: "a1",
          description: "Compacted bare soil along the foundation bed",
          suggestion:
            "Install a 3-foot mulched planting bed with Limelight Hydrangea (3-Gallon) at 3-foot centers",
        },
        {
          id: "a2",
          description: "Thin, declining fescue in the front lawn",
          suggestion:
            "Aerate and overseed with turf-type tall fescue; topdress with compost",
        },
      ],
      botanicalViolations: [],
      visionSummary:
        "Define the foundation line with a tidy hydrangea bed and revive the front lawn — a clean, high-curb-appeal refresh a crew can install in a day.",
      estimatedMaterials: [
        {
          item: "Limelight Hydrangea (3-Gallon)",
          quantity: "7 shrubs",
          estimatedCost: 315,
          geoSpatialVolume: "~45 sq ft bed",
        },
        {
          item: "Double-Shredded Hardwood Mulch",
          quantity: "2 cubic yards",
          estimatedCost: 120,
          geoSpatialVolume: "2 cu yd",
        },
        {
          item: "Turf-Type Tall Fescue Seed",
          quantity: "25 lb",
          estimatedCost: 90,
          geoSpatialVolume: "1,500 sq ft",
        },
      ],
      strategicValue:
        "≈$525 install that lifts curb appeal and sets up a recurring maintenance account.",
  ...
  if (instr.includes("processing continuous voice dictations")) {
    ...
    if (
      /(mulch|bag|bags|shovel|fertiliz|seed|sod|stone|gravel|fuel|gas|pallet|inventory|stock|counted|pine straw|units?)/.test(
        c,
      )
    ) {
      ...
      const item =
        (c.match(
          /(mulch|fertilizer|seed|sod|stone|gravel|fuel|gas|pine straw|shovels?)/,
        ) || [])[1] || "supplies";
      return JSON.stringify({
        intent: "UPDATE_INVENTORY",
        summary:
          `Logged ${qty || ""} ${item}`.replace(/\s+/g, " ").trim() +
          " to inventory.",
        data: { item, quantity: qty ? Number(qty) : null },
      });
    }
    if (
      /(crew|job|site|arrived|delayed|finished|completed|on (my|the) way|en route)/.test(
        c,
      )
    ) {
      return JSON.stringify({
        intent: "UPDATE_CREW_STATUS",
        summary: "Crew/job status update noted.",
        data: {},
      });
    }
    return JSON.stringify({
      intent: "UNKNOWN_OR_UNPARSEABLE",
      summary: "",
      data: {},
    });
  }
...
if (CACHE_FILE && fs.existsSync(CACHE_FILE)) {
  try {
    ...
    geminiCache = new Map<string, string>(
      Object.entries(JSON.parse(fs.readFileSync(CACHE_FILE, "utf-8"))),
    );
    while (geminiCache.size > GEMINI_CACHE_MAX) {
      const oldest = geminiCache.keys().next().value;
      if (oldest === undefined) break;
...
function saveGeminiCache() {
  ...
  _cacheWriteTimer = setTimeout(() => {
    ...
    fs.writeFile(
      CACHE_FILE,
      JSON.stringify(Object.fromEntries(geminiCache)),
      (err) => {
        if (err)
          log.error("Failed to write gemini cache", err, { file: CACHE_FILE });
      },
    );
  }, 2000);
}
...
export function getGeminiCoalescedHits() {
  return geminiFlight.coalesced;
}
export function getGeminiInflightSize() {
  return geminiFlight.size;
}
...
const GEMINI_ACQUIRE_TIMEOUT_MS =
  Number(process.env.GEMINI_ACQUIRE_TIMEOUT_MS) || 20000;
const geminiSemaphore = new Semaphore(GEMINI_MAX_CONCURRENT);
export function getGeminiConcurrency() {
  return {
    active: geminiSemaphore.active,
    queued: geminiSemaphore.queued,
    shed: geminiSemaphore.shed,
  };
}
...
const isOriginDistress = (e: any): boolean => {
  const s = Number(
    e?.status ?? e?.code ?? e?.statusCode ?? e?.response?.status,
  );
...
ai.models.generateContent = async (request: any) => {
  ...
  if (cachedText !== undefined) {
    console.log(
      `[Gemini Cache HIT] ${hash.substring(0, 8)} - Saving compute costs.`,
    );
  ...
  if (geminiFlight.has(hash)) {
    console.log(
      `[Gemini Coalesced] ${hash.substring(0, 8)} - riding in-flight call.`,
    );
  } else {
    console.log(
      `[Gemini Cache MISS] ${hash.substring(0, 8)} - Calling LLM API...`,
    );
  }
  ...
  return geminiFlight.run(hash, async () => {
    ...
    try {
      ...
      response = await geminiSemaphore.run(
        () =>
          geminiBreaker.run(() => originalGenerateContent(request), {
            isFailure: isOriginDistress,
          }),
      ...
      if (e instanceof SemaphoreTimeoutError || e instanceof CircuitOpenError) {
        throw new AiUnavailableError(
          "AI is at capacity — please retry in a moment.",
          "AI_BUSY",
        );
      }
      throw e;
...
function apiCacheSet(key: string, value: { expires: number; data: any }) {
  if (apiCacheStore.size >= API_CACHE_MAX) {
    ...
    for (const [k, v] of apiCacheStore) {
      if (v.expires <= now) apiCacheStore.delete(k);
    }
    if (apiCacheStore.size >= API_CACHE_MAX) {
...
function cacheApiResponse(durationSeconds: number) {
  return (req: any, res: any, next: any) => {
    ...
    res.setHeader("Cache-Control", `private, max-age=${durationSeconds}`);
    ...
    const cacheTenant =
      req.user?.tenantId || req.user?.tenant_id || req.user?.uid || "anon";
    const key = crypto
      .createHash("sha256")
      .update(
        cacheTenant +
...
export async function createApp({ startListening = false } = {}) {
      ...
      try {
        ...
        if (sb) {
          await Promise.race([
            ...
            new Promise((_, rej) =>
              setTimeout(() => rej(new Error("readyz supabase timeout")), 2000),
            ),
          ]);
  ...
  app.use((req: any, res: any, next: any) => {
    ...
    if (p === "/healthz" || p === "/readyz" || p === "/api/health")
      return next();
    ...
    try {
      ...
      /* headers may already be sent on abort */
    }
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (!endpointSecret) {
        console.warn(
          "Stripe webhook invoked but STRIPE_WEBHOOK_SECRET is not set.",
        );
      ...
      try {
        ...
        if (sb) {
          try {
            ...
            if (claimErr) {
              console.error(
                "Stripe idempotency claim failed:",
                claimErr.message,
              );
        ...
        try {
          ...
          const setTenantTier = async (tenantId: string, tier: string) => {
            ...
            const { error } = await sb
              ...
              .eq("id", tenantId);
            if (error)
              throw new Error("setTenantTier failed: " + error.message);
          };
          ...
          const creditInvoice = async (session: any, isDeposit: boolean) => {
            ...
            const { data: inv, error: readErr } = await sb
              .from("invoices")
              .select("amount,data,status,tenant_id,customer_id")
              .eq("id", invId)
              .maybeSingle();
            if (readErr)
              throw new Error("invoice read failed: " + readErr.message);
            if (!inv) return null;
            const paid = (Number(session.amount_total) || 0) / 100;
            const total = Number(inv.amount) || 0;
            const prevPaid = Number(inv.data?.amountPaid) || 0;
            const amountPaid = Math.min(
              Math.round((prevPaid + paid) * 100) / 100,
              total,
            ); // clamp to total
            const payments = Array.isArray(inv.data?.payments)
              ? inv.data.payments
              : [];
            payments.push({
              amount: paid,
            ...
            const { error: updErr } = await sb
              ...
              .eq("id", invId);
            if (updErr)
              throw new Error("invoice update failed: " + updErr.message);
          ...
          switch (event.type) {
            case "checkout.session.completed": {
              ...
              if (md.invoiceId && sb) {
                ...
                if (session.payment_status === "paid") {
                  pendingNotify =
                    (await creditInvoice(session, isDeposit)) || pendingNotify;
                } else {
            ...
            case "customer.subscription.updated": {
              ...
              if (tenantId && tier && sub.status === "active")
              ...
              if (tenantId && sb) {
                try {
                  ...
                  if (Number.isFinite(seats) && seats > 0)
                    patch.seats = Math.floor(seats);
                  if (Object.keys(patch).length)
                    await sb.from("tenants").update(patch).eq("id", tenantId);
                } catch (e: any) {
                  console.warn(
                    "[BILLING] customer/seat persist failed:",
                    e?.message,
                  );
                }
              }
            ...
            case "customer.subscription.deleted": {
              ...
              if (sub.metadata?.tenantId)
                await setTenantTier(sub.metadata.tenantId, "free");
              break;
            }
          ...
          Promise.resolve()
            .then(() =>
              dispatchNotification(
                pendingNotify.tenantId,
                pendingNotify.customerId,
                pendingNotify.event,
      ...
      try {
        ...
        if (process.env.TWILIO_AUTH_TOKEN) {
          try {
            ...
            /* twilio sdk unavailable — fall through */
          }
        }
        ...
        try {
              ...
              if (matches && matches.length === 1) {
                ...
                if (!isControl && String(Body || "").trim()) {
                  ...
                  if (tenant) {
                    ...
                    console.warn(
                      `[SMS inbound] net-new ${last10} but no receptionist tenant for To=${To || "?"}; dropped`,
                    );
                  }
                }
              } else {
                console.warn(
                  `[SMS inbound] ambiguous ${last10} (${matches.length} matches); dropped`,
                );
              }
            })();
            await Promise.race([
          ...
          /* best-effort persistence; still ack to Twilio */
        }
        return xml();
      } catch (e) {
        return xml("<Response/>");
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        let recepCfg: any = {};
        if (sb) {
          const tenant = await withDeadline(
            resolveReceptionistTenant(sb, To),
            3000,
            null,
          );
          if (tenant) {
        ...
        return respond(
          buildAckTwiml(
            "Thanks for calling. We'll be in touch shortly. Goodbye.",
          ),
        );
      }
    },
      ...
      try {
        ...
        if (sb) {
          ...
          if (tenant) {
            ...
            if (r?.extracted?.name)
              ack = `Thanks ${r.extracted.name}! We've got your request and we'll text you right away. Goodbye.`;
          }
        }
        ...
        return respond(
          buildAckTwiml("Thanks. We'll be in touch shortly. Goodbye."),
        );
      }
    },
  );
  ...
  app.post(
    "/api/public/voice/transcription",
    express.urlencoded({ extended: false }),
    async (req: any, res) => {
      try {
        if (!verifyTwilioSignature(req)) return res.sendStatus(403);
        const { From, To, CallSid, TranscriptionText, TranscriptionStatus } =
          req.body || {};
  ...
  app.post(
    ...
    (req: any, res) => {
      try {
        ...
        /* fall through to a valid ack */
      }
      return res
        .type("text/xml")
        .send(
          buildAckTwiml(
            "Thanks. We got your message and we'll text you shortly. Goodbye.",
          ),
        );
    },
  );
  ...
  app.get("/api/security/threats", (req: any, res) => {
    if (REQUIRE_AUTH) {
      ...
      if (
        !req.user ||
        (role !== "admin" && role !== "owner" && !req.user.is_platform_admin)
      ) {
  ...
  app.use((req, res, next) => {
    ...
    const blockedExtensions = [
      ...
      ".ini",
      ".cfg",
      ".conf", // Sensitive Configs
      ".so",
      ".msi",
      ".jar",
      ".war",
      ".ear", // OS & Java Binaries
    ];
    if (blockedExtensions.some((ext) => url.includes(ext))) {
      ...
      return res
        ...
        .json({
          error:
            "This request was blocked for security reasons (restricted file type).",
        });
    }
    ...
    if (
      pathPatterns.some((p) => url.includes(p)) ||
      contentPatterns.some((p) => url.includes(p))
    ) {
      logThreat(req.ip || "", "Injection/Pentest Payload", req.url);
      return res
        .status(403)
        .json({ error: "This request was blocked for security reasons." });
    }
    ...
    (function collect(v: any, budget: { n: number }) {
      ...
      if (typeof v === "string") {
        if (v.length < 4096) {
          leaves.push(v.toLowerCase());
          budget.n--;
        }
        return;
      }
      if (Array.isArray(v)) {
        for (const x of v) collect(x, budget);
        return;
      }
      if (typeof v === "object") {
        for (const k in v) collect(v[k], budget);
      }
    })(req.body, { n: 400 });
    if (leaves.some((s) => contentPatterns.some((p) => s.includes(p)))) {
      logThreat(req.ip || "", "Injection/Pentest Payload", req.url);
      console.warn(
        `[SECURITY] Potential injection detected from IP ${req.ip} on ${req.url}`,
      );
      return res
        .status(403)
        .json({ error: "This request was blocked for security reasons." });
    ...
    if (["POST", "PUT", "PATCH", "DELETE"].includes(req.method)) {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        return res
          .status(415)
          .json({ error: "Requests must use Content-Type: application/json." });
      }
    }
  ...
  const JWT_SECRET =
    ...
    (IS_PROD ? "" : "cutty-dev-only-ephemeral-secret");
  const BASE_URL = process.env.BASE_URL || "http://localhost:3000";
  const PLATFORM_FEE_PCT = Math.max(
    0,
    Number(process.env.PLATFORM_FEE_PCT || 0),
  ); // e.g. 0.02 = 2% platform fee
  ...
  if (IS_PROD) {
    ...
    if (isMockMode) {
      console.warn(
        "[AI] NODE_ENV=production but GEMINI_API_KEY is unset — AI features will serve canned mock output to real customers. Set GEMINI_API_KEY.",
      );
    }
    if (!process.env.JWT_SECRET) {
      console.error(
        "[SECURITY] JWT_SECRET is not set in production. Client-portal magic links will be REJECTED until it is configured.",
      );
    }
    if (!process.env.STRIPE_WEBHOOK_SECRET || !process.env.STRIPE_SECRET_KEY) {
      console.warn(
        "[BILLING] Stripe keys not fully configured in production; payments/webhooks are disabled.",
      );
    }
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
      console.warn(
        "[DATA] SUPABASE_SERVICE_ROLE_KEY not set; tenant provisioning + AI credit metering are disabled until configured.",
      );
    }
  }
  ...
  const authCache = new Map<
    string,
    { uid: string; email?: string; exp: number }
  >();
  ...
  const verifySupabaseToken = async (req: any, res: any, next: any) => {
    ...
    const tokenHeader =
      req.headers["authorization"] || req.headers["x-firebase-auth"];
    ...
    try {
      ...
      if (!sb) {
        ...
        return res
          .status(503)
          .json({
            error: "Auth not configured (SUPABASE_URL / SUPABASE_ANON_KEY)",
          });
      }
      ...
      log.error("Supabase auth middleware error", e, {
        requestId: req?.id,
        path: fullPath,
      });
  ...
  const strictLimiter = rateLimit({
    ...
    message: {
      error:
        "Too many requests to sensitive endpoints. Please try again after 1 hour.",
    },
  });
  ...
  const aiLimiter = rateLimit({
    ...
    message: {
      error:
        "Daily AI generation limit reached (100). Please try again tomorrow.",
    },
  });
  ...
  const writeLimiter = rateLimit({
    ...
    keyGenerator: (req) =>
      (req as any).user?.uid || ipKeyGenerator((req as any).ip),
    message: {
      ...
      contentSecurityPolicy: {
        directives: {
          ...
          scriptSrc: [
            ...
            "https://maps.googleapis.com",
            "https://js.stripe.com",
          ], // Vite needs eval for dev, Stripe/Maps need external scripts
          styleSrc: [
            "'self'",
            "'unsafe-inline'",
  ...
  const TIER_ALLOTMENTS: Record<string, Allotments> = {
    free: {
      seats: numEnv("TIER_FREE_SEATS", 1),
      ai: numEnv("AI_CREDITS_FREE", 50),
      sms: numEnv("TIER_FREE_SMS", 0),
      live_min: numEnv("TIER_FREE_LIVE", 10),
      aerial: numEnv("TIER_FREE_AERIAL", 0),
      pdf: numEnv("TIER_FREE_PDF", 20),
    },
    pro: {
      seats: numEnv("TIER_PRO_SEATS", 3),
      ai: numEnv("AI_CREDITS_PRO", 1000),
      sms: numEnv("TIER_PRO_SMS", 250),
      live_min: numEnv("TIER_PRO_LIVE", 60),
      aerial: numEnv("TIER_PRO_AERIAL", 25),
      pdf: numEnv("TIER_PRO_PDF", 500),
    },
    enterprise: {
      seats: numEnv("TIER_ENT_SEATS", 8),
      ai: numEnv("AI_CREDITS_ENTERPRISE", 10000),
      sms: numEnv("TIER_ENT_SMS", 1500),
      live_min: numEnv("TIER_ENT_LIVE", 300),
      aerial: numEnv("TIER_ENT_AERIAL", 150),
      pdf: numEnv("TIER_ENT_PDF", 500),
    },
  };
  ...
  const BASE_CENTS: Record<string, number> = {
    free: numEnv("TIER_FREE_BASE_CENTS", 0),
    pro: numEnv("TIER_PRO_BASE_CENTS", 24900),
    enterprise: numEnv("TIER_ENT_BASE_CENTS", 64900),
  };
  const SEAT_CENTS: Record<string, number> = {
    free: numEnv("TIER_FREE_SEAT_CENTS", 0),
    pro: numEnv("TIER_PRO_SEAT_CENTS", 2900),
    enterprise: numEnv("TIER_ENT_SEAT_CENTS", 2500),
  };
  ...
  const AI_CREDITS: Record<string, number> = {
    free: TIER_ALLOTMENTS.free.ai,
    pro: TIER_ALLOTMENTS.pro.ai,
    enterprise: TIER_ALLOTMENTS.enterprise.ai,
  };
  ...
  async function loadRollup(
    ...
    try {
      const { data } = await sb
        ...
        .eq("period", period);
      for (const r of data || [])
        if (isMeter(r.meter)) roll[r.meter] = Number(r.quantity) || 0;
    } catch {
      /* fail-open: treat as no usage recorded yet */
    }
    return roll;
  }
  ...
  function effectiveSpendCap(tenant: any): number | null {
    return resolveSpendCapCents(
      tenant?.spend_cap_cents,
      tenant?.tier,
      numEnv("DEFAULT_SPEND_CAP_CENTS", 500000),
    );
  }
  ...
  async function loadRollupSafe(
    sb: any,
    tenantId: string,
    period: string,
  ): Promise<{ roll: Record<Meter, number>; ok: boolean }> {
    const roll = emptyRollup();
    try {
  ...
  function _lkgKey(tenantId: string, period: string) {
    return tenantId + ":" + period;
  }
  function rememberRollup(
    tenantId: string,
    period: string,
    roll: Record<Meter, number>,
  ) {
    const key = _lkgKey(tenantId, period);
    _usageLkg.delete(key);
    _usageLkg.set(key, { ...roll });
    while (_usageLkg.size > _LKG_MAX_KEYS) {
      const oldest = _usageLkg.keys().next().value;
      if (oldest === undefined) break;
      _usageLkg.delete(oldest);
    }
  }
  function lastKnownRollup(
    tenantId: string,
    period: string,
  ): Record<Meter, number> | null {
  ...
  function sendGate(
    res: any,
    gate: { status?: number; body?: any; retryAfterSec?: number },
  ) {
  ...
  async function gateUsage(
      ...
      if (d.status === "spend_cap") {
        return {
          ...
          body: {
            ...
            meter,
            capCents,
            currentOverageCents: d.beforeCents,
            projectedOverageCents: d.beforeCents + d.addCents,
          },
        };
      }
      return {
        ...
        body: {
          error:
            "Cost control is temporarily unavailable — please retry shortly.",
          code: "AI_METERING_UNAVAILABLE",
          meter,
        },
        retryAfterSec: Number(process.env.AI_METERING_RETRY_SEC) || 15,
      };
    }
    ...
    if (!read.ok)
      rememberRollup(tenant.id, period, applyUsage(current, meter, q));
    return { ok: true };
  }
    ...
    try {
      ...
      if (meter === "ai") {
        const { data: t } = await sb
          .from("tenants")
          .select("ai_credits_used,ai_credits_period")
          .eq("id", tenant.id)
          .maybeSingle();
        const usedNow =
          t?.ai_credits_period === period ? t?.ai_credits_used || 0 : 0;
        await sb
          .from("tenants")
          .update({ ai_credits_used: usedNow + q, ai_credits_period: period })
          .eq("id", tenant.id);
      }
    } catch {
      /* fail-open: a metering write must never fail a served request */
    }
  }
  ...
  function meterAiRoute(weight: number | ((req: any) => number)) {
    return async (req: any, res: any, next: any) => {
      ...
      const w =
        typeof weight === "function" ? Number(weight(req)) || 1 : weight;
      const gate = await gateUsage(tenant, "ai", w);
      if (!gate.ok) return sendGate(res, gate);
      const originalJson = res.json.bind(res);
      res.json = (body: any) => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          writeUsage(tenant, "ai", w).catch(() => {});
        }
        return originalJson(body);
      };
      next();
    ...
    try {
      ...
      if (!sb)
        return res
          ...
          .json({
            error:
              "Provisioning unavailable: SUPABASE_SERVICE_ROLE_KEY not configured",
      ...
      if (tenantId) {
        ...
        if (isOwnerSetup) {
          await sb
            .from("tenants")
            .update({ name: companyName, settings: tenantSettings, legal })
            .eq("id", tenantId);
        } else {
          ...
          .upsert(
            {
              ...
              role: effectiveRole,
              email,
              display_name: settings.ownerName || undefined,
              agreements_accepted: true,
              is_platform_admin: isPlatformAdmin && isOwnerSetup,
            },
        ...
        try {
          ...
          if (!existing || existing.length === 0) {
                  ...
                  [
                    {
                      ...
                      first_name: "Gable",
                      last_name: "Jenkins",
                      email: "gable.jenkins@example.com",
                      phone: "601-555-0123",
                      ...
                      is_hoa: true,
                      ai_score: 94,
                      ai_score_label: "Growth Potential",
                      ai_score_reasoning:
                        "Wants holly swap and irrigation check.",
                      notes:
                        "Specific trimming patterns along the driveway approach.",
                      segment: "Platinum",
                      data: {
                        isSample: true,
                        hoaRules: [
                          "No mowing before 9 AM",
                          "Electric equipment only",
                          "Badge ID required",
                        ],
                        propertyDetails: {
                          size: "4.5 acres",
                    ...
                    },
                    {
                      ...
                      first_name: "Marcus",
                      last_name: "Pohl",
                      email: "marcus.pohl@example.com",
                      phone: "601-555-9922",
                      address: "442 Pine Grove Rd",
                      status: "active",
                      is_hoa: false,
                      ai_score: 42,
                      ai_score_label: "Maintenance",
                      ai_score_reasoning: "Standard bi-weekly cuts; small lot.",
                      notes: "Gate code 4420. Dog in the back yard sometimes.",
                      segment: "Base",
                      data: {
                        isSample: true,
                        propertyDetails: {
                    ...
                    {
                      tenant_id: tenantId,
                      company_name: "Cedar Ridge HOA",
                      email: "board@cedarridge.org",
                      phone: "601-555-0103",
                      address: "Cedar Ridge Community",
                      status: "lead",
                      is_hoa: true,
                      ai_score: 82,
                      ai_score_label: "New Lead",
                      notes:
                        "Requested a quote for noise-compliant electric clearing.",
                      data: { isSample: true },
                    },
                  ].map(stampGeocode),
                ),
                ...
                [
                  {
                    tenant_id: tenantId,
                    customer_id: c0,
                    title: "HOA Weekly Mow & Edge",
                    status: "SCHEDULED",
                    date: new Date(Date.now() + 86400000).toISOString(),
                    address: "12 Poplar Springs Dr",
                    data: { isSample: true, client: "Gable Jenkins" },
                  },
                  {
                    tenant_id: tenantId,
                    customer_id: c1,
                    ...
                    address: "442 Pine Grove Rd",
                    progress: 40,
                    data: { isSample: true, client: "Marcus Pohl" },
                  },
                  {
                    tenant_id: tenantId,
                    customer_id: c0,
                    ...
                    data: {
                      isSample: true,
                      client: "Gable Jenkins",
                      snapshotNotes:
                        "Beds mulched, hollies trimmed, irrigation checked.",
                    },
                  },
                ].map(stampGeocode),
              ),
            ...
            await sb.from("leads").insert([
              {
                ...
                address: "120 Poplar Springs Dr",
                prop_size: "4.5 acres",
                match_reason: "High upsell potential for turf irrigation.",
                score: 95,
                data: { isSample: true },
              },
              {
                ...
                prop_size: "2.8 acres",
                match_reason:
                  "Needs a noise-compliant electric clearing quote.",
                score: 82,
                data: { isSample: true },
              },
            ]);
            await sb.from("vendors").insert([
              {
                tenant_id: tenantId,
                name: "Local Supply & Mulch",
                category: "Materials",
                status: "ACTIVE",
                contact: "Bob H.",
                next_delivery: "Mon 8:00 AM",
                data: { isSample: true },
              },
              {
                tenant_id: tenantId,
              ...
              await sb.from("invoices").insert([
                {
                  ...
                  status: "sent",
                  items: [
                    { description: "Spring Cleanup", quantity: 1, rate: 280 },
                  ],
                  data: { isSample: true, client: "Gable Jenkins" },
                },
                {
                  ...
                  items: [
                    {
                      description: "Bi-Weekly Maintenance",
                      quantity: 1,
                      rate: 150,
                    },
                  ],
      ...
      res.json({
        tenantId,
        profile: { tenant_id: tenantId, role: "owner" },
        demoDataLoaded,
      });
    } catch (e: any) {
  ...
  app.post("/api/account/delete", async (req: any, res) => {
    try {
      if (REQUIRE_AUTH && !req.user?.uid)
        return res.status(401).json({ error: "Unauthorized" });
      if ((req.body || {}).confirm !== "DELETE") {
        return res
          .status(400)
          .json({ error: "Confirmation required", code: "CONFIRM_REQUIRED" });
      }
      const sb = getServiceSupabase();
      if (!sb)
        return res
          ...
          .json({
            error:
              "Account deletion unavailable: SUPABASE_SERVICE_ROLE_KEY not configured",
            code: "PROVISION_UNAVAILABLE",
          });
      const tenant = await resolveTenant(req);
      if (!tenant)
        return res
          .status(404)
          .json({ error: "No workspace found for this account." });
      if (tenant.role !== "owner")
        return res
          .status(403)
          .json({ error: "Only the workspace owner can delete the account." });
      ...
      const { error: delTenantErr } = await sb
        .from("tenants")
        .delete()
        .eq("id", tenantId);
      ...
      for (const uid of uids) {
        try {
          ...
          console.warn(
            "[account/delete] auth.admin.deleteUser failed for",
            uid,
            e?.message,
          );
        }
  ...
  app.get("/api/usage/credits", async (req: any, res) => {
    ...
    const used =
      tenant?.ai_credits_period === period ? tenant.ai_credits_used || 0 : 0;
    res.json({
      tier,
      used,
  ...
  app.get("/api/usage/summary", async (req: any, res) => {
    ...
    if (!REQUIRE_AUTH) {
      ...
      return res.json({
        ...
        period: nowPeriod(),
        seats: 1,
        includedSeats: allot.seats,
        baseCents: BASE_CENTS[tier],
        seatCents: SEAT_CENTS[tier],
        seatsCents: bill.seatsCents,
        overageCents: 0,
        projectedTotalCents: bill.totalCents,
        spendCapCents: null,
    ...
    const sb = getServiceSupabase();
    const roll =
      sb && tenant ? await loadRollup(sb, tenant.id, period) : emptyRollup();
    ...
    try {
      ...
      if (
        ...
        if (!Number.isFinite(n) || n < 0)
          return res
            .status(400)
            .json({
              error: "spendCapCents must be a non-negative number or null",
            });
        cap = Math.round(n);
      }
      const tenant = await resolveTenant(req);
      if (!REQUIRE_AUTH || !tenant)
        return res.json({
          simulated: true,
          spendCapCents: cap,
          alerts: alerts || null,
      ...
      if (!["owner", "admin"].includes(tenant.role))
        return res
          .status(403)
          .json({
            error: "Only an owner or admin can change billing controls.",
          });
      const sb = getServiceSupabase();
      if (!sb)
        return res
          ...
          .json({
            error:
              "Billing controls unavailable: SUPABASE_SERVICE_ROLE_KEY not configured",
            code: "PROVISION_UNAVAILABLE",
          });
      ...
      res.json({
        spendCapCents: cap,
        alerts:
          patch.settings?.usageAlerts || tenant.settings?.usageAlerts || null,
      });
    } catch (e: any) {
  ...
  app.use(
    "/api/design/",
    meterAiRoute((req: any) =>
      DESIGN_IMAGE_ROUTE.test(req.originalUrl || req.url || "")
        ? AI_WEIGHT_IMAGE
        : 1,
    ),
  );
  ...
  app.post("/api/public/lead-intake", leadIntakeLimiter, async (req, res) => {
    try {
      ...
      if (!name || (!email && !phone))
        return res
          .status(400)
          .json({ error: "Please provide your name and an email or phone." });
      ...
      if (!sb) {
        console.warn(
          "[lead-intake] Supabase service role not configured; cannot persist lead",
        );
        return res
          .status(503)
          .json({
            error:
              "Online booking is temporarily unavailable. Please call us directly.",
          });
      }
      ...
      if (!UUID_RE.test(id))
        return res
          .status(404)
          .json({
            error: "We couldn't find that business. Please check your link.",
          });
      const { data: tenant } = await sb
        .from("tenants")
        .select("id")
  ...
  app.post("/api/workflows/weather", async (req, res) => {
    try {
      ...
      const city = String(
        req.body?.city || process.env.DEFAULT_WEATHER_CITY || "Austin,US",
      ).slice(0, 80);
      ...
      if (
        ...
        return res.json({
          message:
            "Extreme weather found. Rerouting via integrated portal notification.",
        });
      }
  ...
  app.get("/api/weather", async (req: any, res: any) => {
    ...
    try {
      ...
      const city =
        (q as string) || process.env.DEFAULT_WEATHER_CITY || "Austin,US";
      const url =
        lat && lon
      ...
      const high =
        main === "Rain" || main === "Thunderstorm" || main === "Snow";
      res.json({
        configured: true,
        temp:
          typeof d?.main?.temp === "number" ? Math.round(d.main.temp) : null,
        condition: main,
  ...
  app.post("/api/workflows/followup", async (req, res) => {
    try {
      ...
      if (!to || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to))
        return res
          .status(400)
          .json({ error: "A valid recipient email ('to') is required." });
      const esc = (s: any) =>
        String(s ?? "")
          .replace(/&/g, "&amp;")
          .replace(/</g, "&lt;")
      ...
      const reviewUrl = String(
        req.body?.reviewUrl || tenant?.settings?.googleReviewUrl || "",
      ).trim();
      const footer = esc(
        req.body?.footer || tenant?.settings?.businessAddress || "",
      );
  ...
  app.post("/api/workflows/maintenance", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed maintenance workflow" });
    }
  });
  ...
  app.post("/api/workflows/lead-routing", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed lead-routing workflow" });
    }
  });
    ...
    try {
      ...
      if (!items.length)
        return res
          ...
          .json({
            error:
              "invoice.items is required — this route no longer fabricates demo invoices.",
          });
      ...
      const businessLine = esc(
        inv.businessLine || tenant?.settings?.businessAddress || "",
      );
      ...
      const subtotal = items.reduce(
        (s: number, it: any) =>
          s + (Number(it.rate) || 0) * (Number(it.quantity) || 1),
        0,
      );
      ...
      res.json({
        message: "Invoice PDF generated and emailed.",
      });
    } catch (e: any) {
      res.status(500).json({ error: "Failed generate-invoice-pdf workflow" });
    }
  });
  ...
  app.post("/api/workflows/invoice-chaser", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed invoice chase workflow" });
    }
  });
  ...
  app.post("/api/workflows/chemical-log", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed chemical log workflow" });
    }
  });
  ...
  app.post("/api/workflows/onboarding", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed onboarding workflow" });
    }
  });
  ...
  app.post("/api/workflows/routing", async (req, res) => {
    try {
      ...
      res.json({
        message: "Route optimized via Google Maps Routes API.",
        data,
      });
    } catch (e: any) {
      console.error(e);
  ...
  app.post("/api/workflows/irrigation", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed irrigation workflow" });
    }
  });
  ...
  app.post("/api/stripe/connect", async (req: any, res) => {
    try {
      if (!process.env.STRIPE_SECRET_KEY) {
        return res.json({
          error: "Stripe key missing. Multi-tenant setup simulated.",
        });
      }
      ...
      if (
        ...
        return res
          .status(403)
          .json({ error: "Only an owner or admin can connect billing." });
      }
      const sb = getServiceSupabase();
      const stripe = require("stripe")(process.env.STRIPE_SECRET_KEY);
    ...
    try {
      ...
      if (!process.env.STRIPE_SECRET_KEY) {
        return res.json({
          ...
          simulatedUrl: sameOriginOrDefault(
            successUrl,
            `${BASE_URL}?success=mock`,
          ),
        });
      }
      ...
      if (invoiceId) {
        ...
        try {
          ...
          if (!sb)
            return res
              .status(503)
              .json({ error: "Billing not configured (service role)." });
          const { data: inv } = await sb
            .from("invoices")
            .select("amount,tenant_id,amountPaid")
            .eq("id", invoiceId)
            .maybeSingle();
          if (!inv)
          ...
          if (!inv.amount)
            return res.status(400).json({ error: "Invoice has no amount." });
          ...
          const outstanding = Math.max(
            0,
            Math.round(
              (Number(inv.amount) - (Number((inv as any).amountPaid) || 0)) *
                100,
            ) / 100,
          );
          if (outstanding <= 0)
          ...
          console.error(
            "Supabase lookup failed for invoice price validation:",
            e.message,
          );
          return res
            .status(500)
            .json({ error: "Failed to securely validate invoice price." });
        }
      } else if (!isDeposit) {
        // No invoiceId and not an explicit fixed deposit → refuse rather than trust a client amount.
        return res
          .status(400)
          .json({ error: "invoiceId is required for invoice payments." });
      }
      ...
      const sessionOptions: any = {
        ...
        mode: "payment",
        success_url: sameOriginOrDefault(
          successUrl,
      ...
      if (connectedAccount && PLATFORM_FEE_PCT > 0) {
        sessionOptions.payment_intent_data = {
          application_fee_amount: Math.round(unitAmount * PLATFORM_FEE_PCT),
        };
      }

      const requestOptions: any = connectedAccount
        ? { stripeAccount: connectedAccount }
        : {};
      // Idempotency: a double-submit of the same invoice/deposit charge reuses the same session.
      requestOptions.idempotencyKey = stripeIdempotencyKey(
        isDeposit ? "checkout-deposit" : "checkout-invoice",
        invoiceId || "adhoc",
        unitAmount,
      );
  ...
  const STRIPE_METER_PRICE_ENV: Record<Meter, string> = {
    ...
    live_min: "STRIPE_PRICE_METER_LIVE",
    aerial: "STRIPE_PRICE_METER_AERIAL",
    pdf: "STRIPE_PRICE_METER_PDF",
  };
  ...
  app.post("/api/stripe/subscribe", async (req: any, res) => {
    try {
      ...
      if (!["pro", "enterprise"].includes(tier))
        return res
          .status(400)
          .json({ error: "tier must be 'pro' or 'enterprise'" });
      ...
      const reqSeats = Math.max(
        includedSeats,
        Math.floor(Number(req.body?.seats) || includedSeats),
      );
      const extraSeats = Math.max(0, reqSeats - includedSeats);
      const seatPriceId =
        tier === "enterprise"
          ? process.env.STRIPE_PRICE_SEAT_ENTERPRISE
          : process.env.STRIPE_PRICE_SEAT_PRO;
      // Which metered Prices are configured (reported via Stripe meter events; see the reporter).
      const meteredPrices = METERS.map((m) => ({
        meter: m,
        priceId: process.env[STRIPE_METER_PRICE_ENV[m]],
      })).filter((x) => !!x.priceId);
      ...
      if (!process.env.STRIPE_SECRET_KEY) {
        ...
        return res.json({
          ...
          message:
            "Stripe key missing. Subscription simulated (base + per-seat + metered).",
        });
      }
      const priceId =
        tier === "enterprise"
      ...
      if (!priceId)
        return res
          .status(503)
          .json({
            error: `Stripe price for ${tier} not configured (STRIPE_PRICE_${tier.toUpperCase()})`,
  ...
  app.post("/api/stripe/usage/report", async (req: any, res) => {
    try {
      ...
      const isAdmin = (() => {
        try {
          return !!(
            req.user?.email &&
            process.env.PLATFORM_OWNER_EMAIL &&
            req.user.email === process.env.PLATFORM_OWNER_EMAIL
          );
        } catch {
          return false;
        }
      ...
      if (process.env.USAGE_REPORT_KEY) {
        if (key !== process.env.USAGE_REPORT_KEY && !isAdmin)
          return res.status(403).json({ error: "Forbidden" });
      } else if (REQUIRE_AUTH && !isAdmin) {
        return res
          .status(403)
          .json({
            error: "Set USAGE_REPORT_KEY or call as the platform owner.",
          });
      }
      const period =
        typeof req.body?.period === "string" &&
        /^\d{4}-\d{2}$/.test(req.body.period)
          ? req.body.period
          : nowPeriod();
      const meteredMeters = METERS.filter(
        (m) => !!process.env[STRIPE_METER_PRICE_ENV[m]],
      );
      const sb = getServiceSupabase();
      if (!sb)
        return res.json({
          simulated: true,
          reason: "no service supabase",
          period,
          reported: [],
        });
      ...
      const { data: rows } = await sb
        .from("tenant_usage")
        .select("tenant_id,meter,quantity")
        .eq("period", period);
      const relevant = (rows || []).filter(
        (r: any) => meteredMeters.includes(r.meter) && Number(r.quantity) > 0,
      );
        ...
        try {
          ...
          await stripe.billing.meterEvents.create({
            ...
            payload: {
              ...
              value: String(Math.round(Number(r.quantity))),
            },
          });
          reported.push({
            tenant_id: r.tenant_id,
            meter: r.meter,
            value: Math.round(Number(r.quantity)),
          });
        } catch (e: any) {
          reported.push({
            tenant_id: r.tenant_id,
            meter: r.meter,
            error: e?.message || "report_failed",
          });
        }
      }
  ...
  const RECURRING_INTERVALS: Record<
    string,
    ...
    try {
      ...
      if (!recur)
        return res
          .status(400)
          .json({
            error: `Invalid interval. Use one of: ${Object.keys(RECURRING_INTERVALS).join(", ")}`,
          });
      const amt = Math.round(Number(amount) * 100);
      if (!amt || amt < 50)
        return res
          .status(400)
          .json({ error: "A valid recurring amount is required." });
      if (!process.env.STRIPE_SECRET_KEY) {
        return res.json({
          simulated: true,
          message: "Stripe not configured — recurring plan simulated.",
          interval,
      ...
      if (REQUIRE_AUTH && !connectedAccount) {
        return res
          .status(503)
          .json({
            error: "Connect a Stripe account first (Settings → Stripe).",
      ...
      const session = await stripe.checkout.sessions.create(
        {
          ...
          line_items: [
            {
              price_data: {
                currency: "usd",
                product_data: {
                  name: description || "Recurring landscaping service",
                },
                unit_amount: amt,
                recurring: recur,
              },
          ...
          subscription_data: {
            ...
            ...(PLATFORM_FEE_PCT > 0
              ? { application_fee_percent: Math.round(PLATFORM_FEE_PCT * 100) }
              : {}),
          },
          success_url: sameOriginOrDefault(
            successUrl,
            `${BASE_URL}/admin/invoices?recurring=created`,
          ),
          cancel_url: sameOriginOrDefault(
            cancelUrl,
            `${BASE_URL}/admin/invoices?recurring=canceled`,
          ),
        },
        {
  ...
  const qboConfig = () => {
    ...
    const redirectUri =
      process.env.QBO_REDIRECT_URI || `${BASE_URL}/api/quickbooks/callback`;
  ...
  const qboAccessToken = async (integ: any, opts: { force?: boolean } = {}) => {
    ...
    const notExpired =
      integ.expires_at &&
      new Date(integ.expires_at).getTime() > Date.now() + 60000;
    ...
    const r = await fetchWithTimeout(cfg.tokenUrl, {
      method: "POST",
      headers: {
    ...
    if (sb)
      await sb
        ...
        .update({
          ...
          refresh_token: encryptSecret(newRefresh),
          expires_at,
          updated_at: new Date().toISOString(),
        })
        .eq("id", integ.id);
  ...
  app.get("/api/quickbooks/connect", async (req: any, res) => {
    ...
    if (!cfg.configured)
      return res
        ...
        .json({
          error:
            "QuickBooks is not configured (set QBO_CLIENT_ID / QBO_CLIENT_SECRET).",
          code: "QBO_UNCONFIGURED",
        });
    const tenant = await resolveTenant(req);
    ...
    if (!tenant?.id)
      return res
        .status(401)
        .json({ error: "Sign in before connecting QuickBooks." });
  ...
  app.get("/api/quickbooks/callback", async (req: any, res) => {
    ...
    if (!tenantId)
      return res.redirect(`${BASE_URL}/admin/settings?quickbooks=error`);
    try {
      const basic = Buffer.from(`${cfg.clientId}:${cfg.clientSecret}`).toString(
        "base64",
      );
      ...
      if (!sb)
        throw new Error(
          "SUPABASE_SERVICE_ROLE_KEY required to store the connection.",
        );
      const expires_at = new Date(
        Date.now() + (tok.expires_in || 3600) * 1000,
      ).toISOString();
      await sb.from("integrations").upsert(
        {
          tenant_id: tenantId,
          provider: "quickbooks",
          realm_id: realmId ? String(realmId) : null,
    ...
    try {
      ...
      if (!token)
        return res
          .status(503)
          .json({
            error: "QuickBooks token unavailable; reconnect in Settings.",
            code: "QBO_TOKEN",
          });
      const sb = getServiceSupabase();
      ...
      for (const c of customers || []) {
        ...
        const r = await fetchWithTimeout(
          `${cfg.apiBase}/v3/company/${integ.realm_id}/customer`,
          {
            method: "POST",
            headers: {
        ...
        if (r.ok) synced++;
        else errors.push(`${displayName}: ${r.status}`);
      }
      res.json({
        synced,
  ...
  const qboNormalizeLinks = (raw: any): Link[] => {
    ...
    return raw
      .filter(
        (l: any) => l && (l.entity === "customer" || l.entity === "invoice"),
      )
      .map((l: any) => ({
        localId: String(l.localId ?? ""),
        qboId: String(l.qboId ?? ""),
        entity: l.entity,
  ...
  const qboUpsertLink = (
    ...
    const i = links.findIndex(
      (l) => l.entity === entity && String(l.localId) === lid,
    );
    const next: Link = {
      localId: lid,
      qboId: qid,
  ...
  const qboDegrade = (code: string): { status: number; error: string } => {
    switch (code) {
      case "QBO_RATE_LIMIT":
        return {
          status: 429,
          error: "QuickBooks is rate-limiting requests. Try again in a minute.",
        };
      case "QBO_TOKEN":
        return {
          status: 503,
          error: "QuickBooks token expired. Reconnect in Settings.",
        };
      ...
      case "QBO_UNCONFIGURED":
        return { status: 503, error: "QuickBooks is not configured." };
      case "QBO_PULL_FAILED":
        return {
          status: 502,
          error: "Could not read from QuickBooks right now. Try again shortly.",
        };
      case "QBO_NO_DB":
        return { status: 503, error: "Service database unavailable." };
      default:
        return { status: 500, error: "QuickBooks sync failed." };
    }
  };
  ...
  const qboQuery = async (
    ...
    try {
      ...
      return { ok: false, status: 0, records: [] as any[], network: true };
    }
    if (r.status === 401)
      return {
        ok: false,
    ...
    return {
      ok: true,
      status: 200,
      records: (key ? (qr as any)[key] : []) as any[],
    };
  };
  ...
  const qboQueryAll = async (
    ...
    for (let page = 0; page < maxPages; page++) {
      ...
      if ((res as any).rateLimited)
        return { ok: false, code: "QBO_RATE_LIMIT", partial: true, records };
      if (!res.ok)
        return {
          ok: false,
          code: "QBO_PULL_FAILED",
          partial: records.length > 0,
          records,
        };
  ...
  const qboWrite = async (
    ...
    const rec =
      body?.[entityName] || (body ? body[Object.keys(body)[0]] : null) || null;
  ...
  const qboAdaptLocalInvoice = (row: any, qboId?: string) => ({
    ...
    items: Array.isArray(row.items)
      ? row.items.map((it: any) => ({
          ...
          unitPrice: it.rate ?? it.unitPrice,
          amount: it.amount,
        }))
      : [],
    amount: row.amount,
  ...
  const qboCustomerToLocal = (q: any) => ({
    ...
    company_name:
      q.CompanyName ||
      (!q.GivenName && !q.FamilyName ? q.DisplayName : null) ||
      null,
  ...
  const qboInvoiceToLocal = (q: any, custLocalByQbo: Map<string, string>) => {
    const items = (Array.isArray(q.Line) ? q.Line : [])
      ...
      .map((l: any) => ({
        description:
          l.Description || l.SalesItemLineDetail?.ItemRef?.name || "Item",
        quantity: l.SalesItemLineDetail?.Qty ?? 1,
        rate: l.SalesItemLineDetail?.UnitPrice ?? l.Amount,
        amount: l.Amount,
      }));
    const balance = Number(q.Balance);
    const status = Number.isFinite(balance) && balance <= 0 ? "paid" : "sent";
    const customerId =
      custLocalByQbo.get(String(q.CustomerRef?.value ?? "")) || null;
  ...
  const qboTwoWaySync = async (tenant: any, integ: any) => {
    ...
    if (!integ || !integ.realm_id)
      return { ok: false, code: "QBO_NOT_CONNECTED" } as any;
    const sb = getServiceSupabase();
    ...
    for (const entity of ["Customer", "Invoice", "Payment", "Item"]) {
      ...
      if (!rd.ok)
        return {
          ok: false,
          code: (rd as any).code,
          retryable: (rd as any).code === "QBO_RATE_LIMIT",
        } as any;
      reads[entity] = rd.records;
    }

    // ---- LOAD LOCAL ----
    const { data: custRowsRaw } = await sb
      .from("customers")
      .select("*")
      .eq("tenant_id", tenant.id)
      .limit(1000);
    const { data: invRowsRaw } = await sb
      .from("invoices")
      .select("*")
      .eq("tenant_id", tenant.id)
      .limit(1000);
    ...
    const custRowById = new Map<string, any>(
      custRows.map((r: any) => [String(r.id), r]),
    );
    const invRowById = new Map<string, any>(
      invRows.map((r: any) => [String(r.id), r]),
    );
    ...
    const qboIdForLocal = (entity: string) => {
      ...
      for (const l of links)
        if (l.entity === entity) m.set(String(l.localId), String(l.qboId));
      return m;
    };
    const localForQbo = (entity: string) => {
      const m = new Map<string, string>();
      for (const l of links)
        if (l.entity === entity) m.set(String(l.qboId), String(l.localId));
      return m;
    };

    const localCustomers = custRows.map((r: any) =>
      qboAdaptLocalCustomer(r, qboIdForLocal("customer").get(String(r.id))),
    );
    const localInvoices = invRows.map((r: any) =>
      qboAdaptLocalInvoice(r, qboIdForLocal("invoice").get(String(r.id))),
    );

    const counts: any = {
      customers: qboEmptyEntityCount(),
      invoices: qboEmptyEntityCount(),
      payments: { pulled: 0 },
      items: { pulled: 0 },
    };
    ...
    for (const l of custPlan.alreadyLinked)
      qboUpsertLink(links, l.localId, l.qboId, "customer", l.updatedAt || now);
    for (const L of custPlan.toPush.slice(0, QBO_PUSH_CAP)) {
      const w = await qboWrite(
        cfg,
        integ.realm_id,
        tokenRef.token,
        "Customer",
      ...
      if (w.ok) {
        ...
        if (w.id) {
          ...
          if (row)
            await sb
              .from("customers")
              .update({ data: { ...(row.data || {}), qboId: w.id } })
              .eq("id", row.id)
              .eq("tenant_id", tenant.id);
        }
      } else {
        counts.customers.errors++;
        if ((w as any).rateLimited) break;
      }
    }
    for (const Q of custPlan.toPull.slice(0, QBO_PULL_CAP)) {
      ...
      if (localId && custRowById.has(localId)) {
        ...
        await sb
          .from("customers")
          .update({ ...patch, data: { ...(row.data || {}), qboId: qId } })
          .eq("id", row.id)
          .eq("tenant_id", tenant.id);
        counts.customers.pulled++;
        qboUpsertLink(links, localId, qId, "customer", now);
      } else {
        const { data: ins } = await sb
          ...
          .insert({
            ...
            ...patch,
            status: "active",
            data: { qboId: qId, source: "quickbooks" },
          })
          .select("id")
          .maybeSingle();
        if (ins?.id) {
          counts.customers.pulled++;
          qboUpsertLink(links, ins.id, qId, "customer", now);
        }
      }
    }
    ...
    for (const l of invPlan.alreadyLinked)
      qboUpsertLink(links, l.localId, l.qboId, "invoice", l.updatedAt || now);
    for (const L of invPlan.toPush.slice(0, QBO_PUSH_CAP)) {
      ...
      if (!custQboId) {
        ...
        continue;
      } // no synced customer => cannot create a QBO invoice
      const w = await qboWrite(
        cfg,
        integ.realm_id,
        tokenRef.token,
        "Invoice",
        mapInvoiceToQbo(L, custQboId),
      );
      if (w.ok) {
    ...
    for (const Q of invPlan.toPull.slice(0, QBO_PULL_CAP)) {
      const qId = String(Q?.Id ?? "");
      if (!qId) continue;
      const { amount, status, items, customerId } = qboInvoiceToLocal(
        Q,
        custLocalByQbo,
      );
      ...
      if (localId && invRowById.has(localId)) {
        ...
        await sb
          ...
          .eq("id", row.id)
          .eq("tenant_id", tenant.id);
        counts.invoices.pulled++;
        qboUpsertLink(links, localId, qId, "invoice", now);
      } else if (customerId) {
        ...
        if (ins?.id) {
          counts.invoices.pulled++;
          qboUpsertLink(links, ins.id, qId, "invoice", now);
        }
      } else {
        counts.invoices.skipped++;
      }
    ...
    await sb
      .from("integrations")
      .update({
        data: { ...(integ.data || {}), links, lastSync: summary },
        status: "connected",
        updated_at: now,
      })
      .eq("id", integ.id);
  ...
  app.post("/api/quickbooks/sync-two-way", async (req: any, res) => {
    ...
    try {
      ...
      if (!r.ok) {
        const d = qboDegrade(r.code);
        return res
          .status(d.status)
          .json({ error: d.error, code: r.code, retryable: !!r.retryable });
      }
      res.json(r);
    } catch (e: any) {
  ...
  app.post("/api/quickbooks/sync-nightly", async (req: any, res) => {
    ...
    if (!key)
      return res
        .status(503)
        .json({
          error: "Nightly sync is not enabled.",
          code: "QBO_CRON_DISABLED",
        });
    ...
    if (!sb)
      return res
        .status(503)
        .json({ error: "Service database unavailable.", code: "QBO_NO_DB" });
    try {
  ...
  app.post("/api/agent/hands-free-dictation", aiLimiter, async (req, res) => {
    try {
      ...
      res.json(
        parsed || { intent: "UNKNOWN_OR_UNPARSEABLE", summary: "", data: {} },
      );
    } catch (e: any) {
    ...
    try {
      ...
      if (isMockMode)
        return aiUnavailable(
          res,
          "Text-to-speech requires GEMINI_API_KEY",
          "TTS_UNAVAILABLE",
        );
  ...
  const AGENT_PERSONAS: Record<string, string> = {
    ...
    coach:
      "Encouraging and explanatory — teach the owner the 'why' behind each answer.",
  ...
  app.post("/api/agent/chat", async (req, res) => {
    try {
      ...
      const persona =
        AGENT_PERSONAS[String(settings?.persona || "").toLowerCase()] ||
        AGENT_PERSONAS.warm;
      const model = AGENT_MODELS.has(settings?.model)
        ? settings.model
        : "gemini-2.0-flash";
      const temperature = Math.min(
        1,
        Math.max(0, Number(settings?.temperature ?? 0.7) || 0.7),
      );
  ...
  app.post("/api/crm/draft-proposal", async (req, res) => {
    try {
      ...
      if (!customer || !customer.firstName)
        return res
          .status(400)
          .json({ error: "customer with firstName required" });
      const systemInstruction = `
  ...
  app.post("/api/brain/compress", async (req, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        model: "gemini-2.0-flash",
        contents: [
          "You are an AI memory manager. Summarize the following conversation history into a dense, chronological bulleted list. Preserve all specific dates, measurements, decisions, and constraints. Do not lose factual information. History: " +
  ...
  app.post("/api/brain/query", async (req, res) => {
    try {
      ...
      const model =
        BRAIN_MODELS[String(agentPrefs?.reasoningModel || "").toLowerCase()] ||
        "gemini-2.5-pro";
      const temperature = Math.min(
        1,
        Math.max(0, Number(agentPrefs?.temperature ?? 0.4) || 0.4),
      );
    ...
    try {
      ...
      try {
        ...
        return res
          .status(400)
          .json({
            error: "Could not read website. Ensure it's a valid public URL.",
          });
      }
  ...
  app.get(
    "/api/analytics/telemetry-export",
    cacheApiResponse(60),
    (req, res) => {
  ...
  app.get("/api/revenue/audit", cacheApiResponse(300), async (req, res) => {
    try {
      ...
      const invoicedJobIds = new Set(
        invoices.map((i: any) => i.data?.jobId).filter(Boolean),
      );
      for (const j of jobs) {
        if (
          ...
          continue;
        const value =
          Number(j.revenue ?? j.data?.price ?? j.data?.revenue ?? 0) || 0;
        opportunities.push({
          id: `unbilled-${j.id}`,
          client: j.client || j.data?.client || "Customer",
      ...
      for (const inv of invoices) {
        ...
        if (["paid", "void", "cancelled", "canceled", "draft"].includes(st))
          continue;
        const bal =
          (Number(inv.amount) || 0) - (Number(inv.data?.amountPaid) || 0);
        ...
        if (!isNaN(due) && due < today) {
          opportunities.push({
            id: `overdue-${inv.id}`,
            client: inv.client || inv.data?.client || "Customer",
      ...
      res
        .status(500)
        .json({ error: "Revenue audit failed.", code: "ERR_AUDIT_STALL" });
    }
  });
  ...
  app.post("/api/reports/predictive-maintenance", async (req, res) => {
    try {
      const customers = Array.isArray(req.body?.customers)
        ? req.body.customers
        : [];
      const systemInstruction = `
  ...
  app.post("/api/inventory/check-and-alert", async (req: any, res) => {
    try {
      ...
      const lowStockItems = (data || [])
        .filter(
          (it: any) =>
            it.min_threshold != null &&
            Number(it.quantity) < Number(it.min_threshold),
        )
        .map((it: any) => ({
          name: it.name,
      ...
      if (lowStockItems.length > 0) {
        ...
        if (Date.now() - last > SIX_H) {
          ...
          const items = lowStockItems
            .slice(0, 8)
            .map((i: any) => `${i.name} (${i.current}/${i.min} ${i.unit})`)
            .join(", ");
          Promise.resolve()
            .then(() =>
              dispatchNotification(tenant.id, null, "low_stock", {
    ...
    try {
      ...
      if (!image || typeof image !== "string") {
        return res
          .status(400)
          .json({
            error: "Missing or invalid 'image' (base64 string required).",
          });
      }
      ...
      const effectiveRole =
        (req.user && (req.user.role || req.user.app_role)) || role;
      const isRestrictedRole =
        effectiveRole === "employee" || effectiveRole === "foreman";
      ...
      if (
        settings?.designCatalog &&
        Array.isArray(settings.designCatalog) &&
        settings.designCatalog.length > 0
      ) {
        // Map user's catalog instead of hardcoded
        const customCatalog = settings.designCatalog
          .map(
            (item: any) =>
              `* ${item.type.toUpperCase()}: ${item.name} ${item.description ? `(${item.description})` : ""}`,
          )
          .join("\n");
        catalogText = `\n${customCatalog}`;
      }
      ...
      const hardscapePrompt =
        settings.enableHardscapeBidding || settings.enableWaterFeatureBidding
          ? `
      ...
      try {
        ...
        if (
          catalog.length &&
          Array.isArray(designResult.estimatedMaterials) &&
          !isRestrictedRole
        ) {
        ...
        console.warn("Catalog pricing pass failed:", (e as any)?.message);
      }
  ...
  function describeRegionServer(cx: number, cy: number): string {
    ...
    const vert =
      row === "top" ? "upper" : row === "bottom" ? "lower" : "center";
    const where =
    ...
    try {
      ...
      if (!image || typeof image !== "string") {
        return res
          .status(400)
          .json({
            error: "Missing or invalid 'image' (clean base64 photo required).",
          });
      }
      ...
      if (!regs.length && !description) {
        return res
          .status(400)
          .json({ error: "Provide at least one region or a description." });
      }
      ...
      const instruction = [
        ...
        regionLines.join(" "),
        description
          ? `Overall intent: ${String(description).slice(0, 400)}.`
          : "",
        zonePhrase,
      ...
      if (!generatedImageUrl) {
        return res
          ...
          .json({
            error:
              "The model did not return an image. Try simplifying the request.",
          });
      }
  ...
  app.post("/api/design/edit", aiLimiter, async (req, res) => {
    try {
      ...
      const instructionText = buildEditInstruction({
        instruction,
        regions: regs,
        zone,
      });
      ...
      const cacheKey = crypto
        ...
        .update(
          [
            ...
            refs
              .map((r) => crypto.createHash("sha256").update(r).digest("hex"))
              .join(","),
          ].join("|"),
        )
        .digest("hex");
      ...
      if (!generatedImageUrl) {
        return res
          ...
          .json({
            error:
              "The model did not return an image. Try rephrasing the change.",
          });
      }
  ...
  app.post("/api/design/proposal-pdf", async (req: any, res: any) => {
    try {
      ...
      const safeImg = (s: any) =>
        typeof s === "string" &&
        /^data:image\/[a-z0-9.+-]+;base64,[a-z0-9+/=\s]+$/i.test(s)
          ? s
          : "";
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="design-proposal.pdf"`,
      );
      res.send(pdf);
    } catch (e: any) {
    ...
    try {
      ...
      if (!beforeImage || typeof beforeImage !== "string") {
        return res
          ...
          .json({
            error:
              "Missing or invalid 'beforeImage' (clean base64 photo required).",
          });
      }
      if (!afterImage || typeof afterImage !== "string") {
        return res
          ...
          .json({
            error:
              "Missing or invalid 'afterImage' (clean base64 photo required).",
          });
      }
      ...
      const beforeData = beforeImage.includes(",")
        ? beforeImage.split(",")[1]
        : beforeImage;
      const beforeMime = beforeImage.includes(";")
        ? beforeImage.split(";")[0].split(":")[1]
        : "image/jpeg";
      const afterData = afterImage.includes(",")
        ? afterImage.split(",")[1]
        : afterImage;
      const afterMime = afterImage.includes(";")
        ? afterImage.split(";")[0].split(":")[1]
        : "image/jpeg";
  ...
  async function runDeepResearch(prompt: string): Promise<string> {
    ...
    const sources = [
      ...new Set(chunks.map((c: any) => c?.web?.uri).filter(Boolean)),
    ];
    if (sources.length) {
      text +=
        "\n\n---\n**Sources**\n" +
  ...
  app.post("/api/research/start", aiLimiter, async (req, res) => {
    try {
      if (isMockMode)
        return aiUnavailable(
          res,
          "Deep research requires GEMINI_API_KEY",
      ...
      runDeepResearch(prompt.slice(0, 4000))
        .then((report) =>
          researchJobs.set(id, { status: "completed", report, ts: Date.now() }),
        )
        .catch((e: any) => {
      ...
      for (const [k, v] of researchJobs)
        if (Date.now() - v.ts > 3600_000) researchJobs.delete(k);
  ...
  app.post("/api/research/status", aiLimiter, async (req, res) => {
    try {
      if (isMockMode)
        return res.json({
          status: "completed",
          report: "Deep research requires GEMINI_API_KEY (mock mode).",
        });
      ...
      if (!job)
        return res.json({
          status: "failed",
          report: "That research job expired or was not found.",
        });
      if (job.status === "completed")
        return res.json({ status: "completed", report: job.report || "" });
      if (job.status === "failed")
        return res.json({
          status: "failed",
          report: job.error || "Research failed.",
        });
  ...
  app.post("/api/marketing/video-status", aiLimiter, async (req, res) => {
    try {
      if (isMockMode)
        return aiUnavailable(
          res,
          "Promo video generation requires GEMINI_API_KEY",
          "VIDEO_UNAVAILABLE",
        );
  ...
  app.post("/api/design/tiers", cacheApiResponse(300), async (req, res) => {
    try {
      ...
      try {
        ...
        console.warn("Tiers pricing pass failed:", (e as any)?.message);
      }
      res.json({ ...designResult, mock: isMockMode });
    } catch (error: any) {
      ...
      const lineItems =
        Array.isArray(items) && items.length
          ? items.map((it: any) => {
              ...
              return {
                desc: esc(it?.description || "Service"),
                qty,
                rate,
                lineTotal,
              };
  ...
  app.post("/api/integration/keep", async (req, res) => {
    try {
      ...
      const keepRes = await fetchWithTimeout(
  ...
  app.post("/api/integration/chat", async (req, res) => {
    try {
      ...
      const chatRes = await fetchWithTimeout(
        `https://chat.googleapis.com/v1/spaces/${spaceName || "messages"}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  app.post("/api/reviews/reply", async (req: any, res) => {
    try {
      ...
      if (!reviewId || !reply || !String(reply).trim()) {
        return res
          .status(400)
          .json({ error: "reviewId and reply are required" });
      }
        ...
        if (sb) {
          ...
          if (tenant) {
            const { data: row } = await sb
              ...
              .maybeSingle();
            externalName =
              row?.data?.googleReviewName || row?.data?.externalId || "";
          }
        }
      } catch (e) {
        /* fall through to honest "not configured" */
      }

      const isGoogle = String(platform || "")
        .toLowerCase()
        .includes("google");
      if (gbpToken && externalName && isGoogle) {
        try {
          const r = await fetchWithTimeout(
            `https://mybusiness.googleapis.com/v4/${externalName}/reply`,
            {
              method: "PUT",
              headers: {
                Authorization: `Bearer ${gbpToken}`,
                "Content-Type": "application/json",
              },
          ...
          console.error(
            "[reviews/reply] Google API error",
            r.status,
            t.slice(0, 500),
          );
          return res.json({
            ...
            configured: true,
            reason:
              "Google rejected the reply. Check your Business Profile connection.",
          });
        } catch (e: any) {
          ...
          return res.json({
            posted: false,
            configured: true,
            reason: "Could not publish the reply right now.",
          });
        }
      }
      return res.json({
        posted: false,
        configured: false,
        reason: "Connect a Google Business Profile to publish replies.",
      });
    } catch (error: any) {
  ...
  app.post("/api/reviews/ingest", async (req: any, res) => {
    try {
      ...
      const placeId = String(
        req.body?.googlePlaceId || settings.googlePlaceId || "",
      ).trim();
      const yelpBusinessId = String(
        req.body?.yelpBusinessId || settings.yelpBusinessId || "",
      ).trim();
      ...
      async function pullGoogle(): Promise<IngestedReview[]> {
        ...
        if (!r.ok) {
          ...
          console.error(
            "[reviews/ingest] Google Places error",
            r.status,
            body.slice(0, 300),
          );
        ...
        return [
          {
            ...
            rating: 5,
            text: "Sample review (demo data) - connect Google or Yelp in Settings to sync your real reviews.",
            author: "Sample Customer",
            createdAt: new Date(t - 2 * day).toISOString(),
          },
          {
            source: "google",
            externalId: "sample:google:2",
            rating: 4,
            text: "Sample review (demo data). Real ratings and counts populate here once a review source is connected.",
            author: "Sample Customer",
            createdAt: new Date(t - 9 * day).toISOString(),
          },
          {
            source: "yelp",
            externalId: "sample:yelp:1",
            rating: 5,
            text: "Sample review (demo data) from Yelp. Placeholder content, not a real customer.",
            author: "Sample Customer",
            createdAt: new Date(t - 20 * day).toISOString(),
          },
        ];
      }
      ...
      if (googleConnected) {
        try {
          googleReviews = await pullGoogle();
        } catch (e: any) {
          googleError = true;
        }
      }
      ...
      const isSampleRow = (rv: IngestedReview) =>
        usedSample || String(rv.externalId).startsWith("sample:");
      ...
      if (persisted) {
        ...
        const { data: existingRows } = await sb
          .from("reviews")
          .select("id, rating, created_at, data")
          .eq("tenant_id", tenant.id);
        ...
        if (plan.toInsert.length) {
          ...
          if (error)
            console.error("[reviews/ingest] insert failed:", error?.message);
        ...
        for (const u of plan.toUpdate) {
          ...
          const { error } = await sb
            ...
            .eq("tenant_id", tenant.id);
          if (error)
            console.error("[reviews/ingest] update failed:", error?.message);
          else updated += 1;
        }
        ...
        rollup = rollupRatings(
          incoming.map((r) => ({ rating: r.rating, createdAt: r.createdAt })),
          nowISO,
        );
      }
          ...
          responseSchema: {
            ...
            properties: {
              ...
              category: {
                type: Type.STRING,
                description:
                  "One of: Fuel, Supplies, Maintenance, Chemicals, Marketing, Other",
              },
              date: {
                type: Type.STRING,
  ...
  const DOC_UNDERSTANDING_MODEL =
    process.env.GEMINI_DOC_MODEL || "gemini-2.0-flash";
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        amount: {
                          type: Type.NUMBER,
                          description:
                            "Extended line total (the money for this row)",
                        },
                        quantity: {
                          type: Type.NUMBER,
                          description: "Quantity for this row",
                        },
                      },
                    },
                  },
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                documentType: {
                  type: Type.STRING,
                  description: "e.g. Service Contract, Building Permit",
                },
                parties: {
                  type: Type.ARRAY,
                ...
                totalValue: {
                  type: Type.NUMBER,
                  description: "Contract value / permit fee if stated",
                },
                scopeOfWork: {
                  type: Type.STRING,
                  description: "Summary of the work / purpose",
                },
                keyTerms: {
                  type: Type.ARRAY,
                  description: "Notable terms",
                  items: { type: Type.STRING },
                },
                obligations: {
                  type: Type.ARRAY,
                  description: "Obligations / requirements",
                  items: { type: Type.STRING },
                },
                permitNumber: {
                  type: Type.STRING,
                  description: "Permit number (permits only) or null",
                },
                issuingAuthority: {
                  type: Type.STRING,
                  description: "Issuing authority (permits only) or null",
                },
                jurisdiction: {
                  type: Type.STRING,
                  description: "Jurisdiction / county or null",
                },
              },
  ...
  app.post("/api/crm/enrich", cacheApiResponse(300), async (req, res) => {
    try {
      ...
      const zoneLabel = z.zone
        ? `${z.zone}${z.approx ? " (approx)" : ""}`
        : "Unknown";
      const loc =
        [addr, city, state].filter(Boolean).join(", ") || "the customer's area";
      ...
      if (isMockMode) {
        return res.json({
          ...
          strategicInsight:
            "Connect a Gemini key for AI-grounded property enrichment.",
          simulated: true,
        });
      }
...
{"estimatedPropertyValue": number|null, "hardinessZone": "string", "soilComposition": "Clay|Sandy|Loamy|Mixed|Unknown", "neighborhoodGrowth": "Rising|Stable|Declining", "upsellProbability": number, "strategicInsight": "one sentence upsell logic tied to this property type/area"}`;
      ...
      if (
        z.zone &&
        (!data.hardinessZone || /unknown/i.test(String(data.hardinessZone)))
      ) {
  ...
  app.post(
    "/api/outbound/draft-personalized-campaign",
    aiLimiter,
    async (req, res) => {
      try {
        ...
        if (JSON.stringify(customers).length > 200000) {
          return res
            .status(400)
            .json({
              error: "Payload Too Large: Max customer batch size exceeded.",
            });
        }
  ...
  app.post("/api/translate", aiLimiter, async (req, res) => {
    try {
      ...
      if (
        ...
        !/^[A-Za-z\- ()\.]+$/.test(targetLanguage)
      ) {
        return res
          .status(400)
          .json({ error: "Invalid target language format." });
      }
        ...
        contents: [
          {
            ...
            parts: [
              ...
              {
                text:
                  prompt || "Analyze this media and describe key information.",
              },
            ],
          },
        ],
  ...
  app.post("/api/playground/generate-music", async (req, res) => {
    try {
      ...
      res.json({
        text:
          "Music generation request succeeded. Response: " +
          (response.text || "Audio generated."),
      });
    } catch (e: any) {
      return handleAiError(res, e);
    }
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          const r = (await fetchSafeExternal(url, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body,
            timeoutMs: 8000,
      ...
      console.error(
        "[automations/webhook] delivery failed after",
        maxAttempts,
        "attempts:",
        lastErr,
      );
  ...
  app.post("/api/sms/send", async (req: any, res) => {
    try {
      ...
      if (digits.length < 10 || digits.length > 15) {
        return res
          .status(400)
          .json({ error: "A valid destination phone number is required." });
      }
      ...
      if (REQUIRE_AUTH) {
        ...
        if (!owns) {
          return res
            ...
            .json({
              error:
                "You can only text a phone number saved to one of your customers.",
            });
        }
      }
      ...
      const persistOutbound = async () => {
        try {
          ...
          console.warn("[sms/send] persist failed:", e?.message);
        }
      };
      ...
      if (
        ...
        console.warn(
          "[TWILIO SIMULATION] Mocking SMS send because credentials are not set.",
        );
    ...
    try {
      ...
      if (!JWT_SECRET)
        return res
          .status(503)
          .json({
            error: "Magic links unavailable: JWT_SECRET not configured",
      ...
      if (tenant?.id) {
        ...
        if (sb) {
          ...
          if (!cust || cust.tenant_id !== tenant.id) {
            return res
              .status(403)
              .json({ error: "Client not found in your workspace" });
          }
        }
        tenantId = tenant.id;
      ...
      const token = jwt.sign(
        { clientId, tenantId, email, scope: "portal" },
        JWT_SECRET,
        { expiresIn: "7d" },
      );
      const magicLink =
        req.protocol + "://" + req.get("host") + "/portal/auth/" + token;
  ...
  app.post("/api/auth/magic-link/validate", (req, res) => {
    try {
      ...
      // Restrict accepted signature algorithm to prevent algorithm confusion attacks (e.g. HS256 vs RS256/none)
  ...
  const verifyPortalToken = (req: any) => {
    ...
    try {
      // Restrict accepted signature algorithm to prevent algorithm confusion attacks
  ...
  app.get("/api/portal/data", strictLimiter, async (req: any, res: any) => {
    ...
    if (!sb)
      return res
        .status(503)
        .json({
          error: "Portal data unavailable (service role not configured)",
        });
    try {
      ...
      const [custR, jobsR, invR, msgR, dvR] = await Promise.all([
        sb
          .from("customers")
          .select(
            "id,first_name,last_name,company_name,address,email,phone,tenant_id",
          )
          .eq("id", clientId)
          .maybeSingle(),
        sb
          .from("jobs")
          .select("id,title,status,date,address,data")
          .eq("customer_id", clientId)
          .order("date", { ascending: false })
          .limit(50),
        sb
          .from("invoices")
          .select("id,amount,status,date,due_date,items,data,is_archived")
          .eq("customer_id", clientId)
        ...
        sb
          ...
          .select("id,summary,before_url,after_url,proposal,created_at")
          .eq("customer_id", clientId)
          .order("created_at", { ascending: false })
          .limit(10),
      ]);
      ...
      res.json({
        ...
        jobs: (jobsR.data || []).map((j: any) => ({
          ...
          date: j.date,
          address: j.address,
          notes: j.data?.snapshotNotes || null,
          departurePhotoUrl: j.data?.departurePhotoUrl || null,
          completedAt: j.data?.completedAt || null,
        })),
        invoices: (invR.data || [])
          ...
          .map((i: any) => ({
            ...
            dueDate: i.due_date,
            items: i.items,
            client: i.data?.client || null,
            amountPaid: Number(i.data?.amountPaid) || 0,
            number: i.data?.number || null,
          })),
        messages: msgR.data || [],
        designs: (dvR.data || []).map((d: any) => ({
          id: d.id,
          summary: d.summary,
          beforeUrl: d.before_url,
          afterUrl: d.after_url,
          proposal: d.proposal,
  ...
  app.post("/api/portal/message", strictLimiter, async (req: any, res: any) => {
    ...
    try {
      ...
      Promise.resolve()
        .then(() =>
          dispatchNotification(cust.tenant_id, tok.clientId, "new_message", {
            channel: "portal",
  ...
  const createInvoiceCheckout = async (
    ...
    const purpose =
      extraMetadata?.type === "deposit" ? "invoice-deposit" : "invoice-balance";
    requestOptions.idempotencyKey = stripeIdempotencyKey(
      purpose,
      inv.id,
      unitAmount,
  ...
  app.post(
    ...
    async (req: any, res: any) => {
      ...
      if (!tok)
        return res
          .status(401)
          .json({ error: "Invalid or expired portal link" });
      const {
        invoiceId,
        successUrl,
        cancelUrl,
      ...
      try {
        const { data: inv } = await sb
          .from("invoices")
          .select("amount,tenant_id,customer_id,status,data")
          .eq("id", invoiceId)
          .maybeSingle();
        if (!inv) return res.status(404).json({ error: "Invoice not found" });
        if (inv.customer_id !== tok.clientId)
        ...
        if (["paid", "void", "cancelled", "canceled"].includes(invStatus)) {
          return res
            .status(409)
            .json({ error: "This invoice is already settled" });
        }
        if (!process.env.STRIPE_SECRET_KEY) {
          return res.json({
            error: "Stripe key missing. Payment simulated.",
        ...
        const unitAmount = Math.round(chargeDollars * 100);
        if (!unitAmount || unitAmount < 50)
          return res.status(400).json({ error: "Nothing due on this invoice" });
        const sessionOptions: any = {
          payment_method_types: ["card", "us_bank_account"],
          metadata: { invoiceId },
          line_items: [
  ...
  app.post(
    ...
    async (req: any, res: any) => {
      ...
      try {
        ...
        if (dv.customer_id !== tok.clientId)
    ...
    async (req: any, res: any) => {
      ...
      try {
        const { data: dv } = await sb
          .from("customer_design_visions")
          .select(
            "id,customer_id,tenant_id,summary,before_url,after_url,proposal",
          )
          .eq("id", proposalId)
          .maybeSingle();
        if (!dv || dv.customer_id !== tok.clientId)
          return res.status(404).json({ error: "Proposal not found" });
        if (tok.tenantId && dv.tenant_id && dv.tenant_id !== tok.tenantId)
          return res.status(403).json({ error: "Scope mismatch" });
        ...
        if (estId) {
          const { data: inv } = await sb
            .from("invoices")
            .select("id,amount,status,data,customer_id")
            .eq("id", estId)
            .maybeSingle();
          ...
          Promise.resolve()
            .then(() =>
              dispatchNotification(
                ...
                "proposal_viewed",
                {
                  viewCount: patch.viewCount,
                  summary: dv.summary || patch.title || null,
                },
              ),
            )
            .catch(() => {});
  ...
  app.post(
    "/api/portal/proposal/select-tier",
    strictLimiter,
    async (req: any, res: any) => {
      const tok = verifyPortalToken(req);
      if (!tok)
      ...
      if (!proposalId || !tierId)
        return res
          .status(400)
          .json({ error: "proposalId and tierId are required" });
      try {
        const { data: dv } = await sb
          .from("customer_design_visions")
        ...
        if (!dv || dv.customer_id !== tok.clientId)
          return res.status(404).json({ error: "Proposal not found" });
        const base: any = dv.proposal || {};
        const tier = (Array.isArray(base.tiers) ? base.tiers : []).find(
          (t: any) => t?.id === tierId,
        );
        ...
        if (!estId)
          return res
            .status(400)
            .json({ error: "This proposal has no estimate to accept." });

        const { data: inv } = await sb
          .from("invoices")
          .select("id,customer_id,status,data,amount")
          .eq("id", estId)
          .maybeSingle();
        if (!inv || inv.customer_id !== tok.clientId)
        ...
        if (["paid", "accepted", "void", "cancelled", "canceled"].includes(s)) {
          return res
            .status(409)
            .json({ error: "This estimate can no longer be changed." });
        }
        ...
        await sb
          ...
          .update({
            ...
            data: {
              ...(inv.data || {}),
              proposalTier: { id: tier.id, name: tier.name, price },
            },
          })
          .eq("id", estId);
        await sb
          .from("customer_design_visions")
          .update({ proposal: { ...base, selectedTier: tierId } })
          .eq("id", dv.id);
        res.json({
          success: true,
          amount: price,
          tier: { id: tier.id, name: tier.name, price },
        });
      } catch (e: any) {
  ...
  app.post(
    ...
    async (req: any, res: any) => {
      ...
      const sb = getServiceSupabase();
      if (!sb) return res.status(503).json({ error: "Portal unavailable" });
      const { invoiceId, signerName, signatureDataUrl, acceptedTier } =
        req.body || {};
      const name = String(signerName || "")
        .trim()
        .slice(0, 120);
      if (!invoiceId || !name)
        return res
          .status(400)
          .json({ error: "invoiceId and your name are required to sign." });
      ...
      if (typeof signatureDataUrl === "string" && signatureDataUrl) {
        if (
          ...
          signatureDataUrl.length > 200_000
        ) {
          return res
            .status(400)
            .json({ error: "Signature image invalid or too large." });
        }
      ...
      try {
        const { data: inv } = await sb
          .from("invoices")
          .select("id,customer_id,tenant_id,status,data,amount")
          .eq("id", invoiceId)
          .maybeSingle();
        if (!inv) return res.status(404).json({ error: "Estimate not found" });
        if (inv.customer_id !== tok.clientId)
          return res.status(403).json({ error: "Not your estimate" });
        if (
          ["paid", "accepted"].includes(String(inv.status || "").toLowerCase())
        ) {
          return res
            .status(409)
            .json({ error: `This estimate is already ${inv.status}.` });
        }
        const fwd = String(req.headers["x-forwarded-for"] || "")
          .split(",")[0]
          .trim();
        ...
        const dep = computeDeposit(Number(inv.amount) || 0, {
          depositAmount: inv.data?.depositAmount,
          depositPct: inv.data?.depositPct,
        });
          ...
          if (process.env.STRIPE_SECRET_KEY) {
            try {
              const session = await createInvoiceCheckout(
                sb,
                { id: invoiceId, tenant_id: inv.tenant_id },
                dep.amount,
                `Deposit — Estimate ${String(invoiceId).slice(0, 6)}`,
                { type: "deposit" },
                req.body?.successUrl,
                req.body?.cancelUrl,
              );
  ...
  app.post(
    ...
    async (req: any, res: any) => {
      ...
      try {
        ...
        if (inv.customer_id !== tok.clientId)
          return res.status(403).json({ error: "Not your estimate" });
        if (inv.data?.deposit?.status === "paid")
        ...
        if (!dep.required)
          return res
            .status(400)
            .json({ error: "No deposit is due on this estimate." });
        if (!process.env.STRIPE_SECRET_KEY) {
          return res.json({
            simulated: true,
            simulatedUrl: sameOriginOrDefault(
    ...
    async (req: any, res: any) => {
      ...
      try {
        const { data: inv } = await sb
          .from("invoices")
          .select(
            "id,amount,status,date,due_date,items,data,customer_id,tenant_id",
          )
          .eq("id", invoiceId)
          .maybeSingle();
        if (!inv) return res.status(404).json({ error: "Invoice not found" });
        ...
        const hasBreakdown =
          d.subtotal != null &&
          (Number(d.taxAmount) > 0 || Number(d.discount) > 0);
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="invoice-${String(inv.id).slice(0, 6)}.pdf"`,
        );
        res.send(pdf);
      } catch (e: any) {
  ...
  const cleanProposalTiers = (raw: any, fallbackAmount: number): any[] => {
    ...
    return arr.map((t: any, i: number) => ({
      id: String(t?.id || ["good", "better", "best"][i] || `tier${i}`).slice(
        0,
        40,
      ),
      ...
      bullets: Array.isArray(t?.bullets)
        ? t.bullets.slice(0, 12).map((b: any) => String(b).slice(0, 160))
        : [],
    }));
  };
  ...
  app.post("/api/proposals/send", async (req: any, res: any) => {
    ...
    if (!sb)
      return res
        .status(503)
        .json({ error: "Proposals unavailable (service role not configured)" });
    try {
      const tenant = await resolveTenant(req);
      if (REQUIRE_AUTH && !tenant)
        return res.status(401).json({ error: "Unauthorized" });
      ...
      let customerId: string | null = b.customerId
        ? String(b.customerId)
        : null;
      ...
      if (invoiceId) {
        const { data: inv } = await sb
          .from("invoices")
          .select("id,amount,customer_id,tenant_id")
          .eq("id", invoiceId)
          .maybeSingle();
        if (!inv) return res.status(404).json({ error: "Estimate not found" });
        if (tenantId && inv.tenant_id && inv.tenant_id !== tenantId)
          return res.status(403).json({ error: "Not in your workspace" });
      ...
      if (!customerId)
        return res
          ...
          .json({
            error:
              "Link this estimate to a customer before sending a proposal.",
          });

      // Verify the customer is in the owner's tenant (and resolve tenant in demo mode).
      const { data: cust } = await sb
        .from("customers")
        .select("id,tenant_id,first_name,last_name,company_name")
        .eq("id", customerId)
        .maybeSingle();
      if (!cust) return res.status(404).json({ error: "Customer not found" });
      if (tenantId && cust.tenant_id !== tenantId)
        return res.status(403).json({ error: "Not in your workspace" });
      tenantId = tenantId || cust.tenant_id;
      if (REQUIRE_AUTH && tenant && tenantId && tenant.id !== tenantId)
        return res.status(403).json({ error: "Not in your workspace" });
      ...
      if (designId) {
        ...
        if (
          dv &&
          dv.customer_id === customerId &&
          (!tenantId || dv.tenant_id === tenantId)
        )
          existing = dv;
      }
      ...
      const summary =
        b.summary != null
          ? String(b.summary).slice(0, 400)
          : existing?.summary || null;
      const beforeUrl =
        b.beforeUrl != null
          ? String(b.beforeUrl)
          : existing?.before_url || prev.beforeUrl || null;
      const afterUrl =
        b.afterUrl != null
          ? String(b.afterUrl)
          : existing?.after_url || prev.afterUrl || null;
      const proposal = {
        ...prev,
        title:
          b.title != null
            ? String(b.title).slice(0, 160)
            : prev.title || summary || "Your Proposal",
        summary,
        tiers,
        recommendedTier: b.recommendedTier
          ? String(b.recommendedTier).slice(0, 40)
          : prev.recommendedTier || "better",
  ...
  app.get("/api/proposals/engagement", async (req: any, res: any) => {
    ...
    try {
      ...
      const estIds = Array.from(
        new Set(
          sent.map((r: any) => r.proposal?.estimateInvoiceId).filter(Boolean),
        ),
      );
      ...
      if (estIds.length) {
        ...
        for (const inv of invs || []) {
          const s = String(inv.status || "").toLowerCase();
          signedByInvoice[inv.id] =
            s === "accepted" || s === "paid" || !!inv.data?.signature;
        }
      }
      ...
      const proposals = sent.map((r: any) => {
        ...
        const signed = estimateInvoiceId
          ? !!signedByInvoice[estimateInvoiceId]
          : p.status === "signed" || p.status === "accepted" || !!p.approved;
        return {
          proposalId: r.id,
      ...
      for (const p of proposals)
        if (p.estimateInvoiceId) byInvoice[p.estimateInvoiceId] = p;
  ...
  app.get("/api/team", async (req: any, res: any) => {
    ...
    if (!sb)
      return res
        ...
        .json({
          error:
            "Team management unavailable (SUPABASE_SERVICE_ROLE_KEY not set)",
          code: "PROVISION_UNAVAILABLE",
        });
    const tenant = await resolveTenant(req);
  ...
  app.post("/api/team/invite", writeLimiter, async (req: any, res: any) => {
    ...
    if (!sb)
      return res
        .status(503)
        .json({
          error: "Invites unavailable (SUPABASE_SERVICE_ROLE_KEY not set)",
          code: "PROVISION_UNAVAILABLE",
        });
    const tenant = await resolveTenant(req);
    ...
    if (tenant.role !== "owner")
      return res
        .status(403)
        .json({ error: "Only the owner can invite team members." });
    const email = String(req.body?.email || "")
      .trim()
      .toLowerCase();
    const role = ["employee", "foreman"].includes(req.body?.role)
      ? req.body.role
      : "employee";
  ...
  app.post("/api/team/remove", async (req: any, res: any) => {
    ...
    if (!sb)
      return res
        .status(503)
        .json({
          error: "Team management unavailable",
          code: "PROVISION_UNAVAILABLE",
        });
    const tenant = await resolveTenant(req);
    if (!tenant || tenant.role !== "owner")
      return res
        .status(403)
        .json({ error: "Only the owner can remove team members." });
    ...
    if (!m)
      return res
        .status(404)
        .json({ error: "Member not found in your workspace." });
    await sb
      .from("profiles")
      .delete()
      .eq("firebase_uid", memberId)
      .eq("tenant_id", tenant.id);
    try {
      await sb.auth.admin.deleteUser(memberId);
    } catch {}
    res.json({ success: true });
  });
  ...
  const requirePlatformAdmin = async (req: any, res: any) => {
    ...
    if (!sb) {
      res
        ...
        .json({
          error:
            "Admin console unavailable (SUPABASE_SERVICE_ROLE_KEY not set)",
        });
      return null;
    }
  ...
  app.post("/api/admin/tenants/:id/tier", async (req: any, res: any) => {
    ...
    const tier = ["free", "pro", "enterprise"].includes(req.body?.tier)
      ? req.body.tier
      : null;
  ...
  async function sendSmsRaw(to: string, message: string) {
    const digits = String(to || "").replace(/\D/g, "");
    if (digits.length < 10 || digits.length > 15)
      return { sent: false, simulated: false, reason: "invalid_phone" };
    if (
      !process.env.TWILIO_ACCOUNT_SID ||
      !process.env.TWILIO_AUTH_TOKEN ||
      !process.env.TWILIO_PHONE_NUMBER
  ...
  function buildNotifContent(
    ...
    const sms =
      (body.length > 300 ? body.slice(0, 297) + "…" : body) + smsFooter;
  ...
  async function dispatchNotification(
    ...
    try {
      if (!NOTIF_AUDIENCE[event]) {
        out.reason = "unknown_event";
        return out;
      }
      const sb = getServiceSupabase();
      if (!sb || !tenantId) {
        out.reason = "no_supabase_or_tenant";
        return out;
      }

      const { data: tenant } = await sb
        .from("tenants")
        .select("id,name,settings,tier,spend_cap_cents")
        .eq("id", tenantId)
        .maybeSingle();
      ...
      const meterTenant = {
        id: tenantId,
        tier: (tenant as any)?.tier || "free",
        spend_cap_cents: (tenant as any)?.spend_cap_cents ?? null,
      };
      ...
      if (audience === "owner") {
        ...
        recipientEmail =
          tprefs.ownerEmail || (tenant?.settings as any)?.ownerEmail || null;
        recipientPhone = tprefs.ownerPhone || null;
        prefs = {
          channels: tprefs.channels || {},
          quietHours: tprefs.quietHours || null,
          eventMutes: Array.isArray(tprefs.eventMutes) ? tprefs.eventMutes : [],
        };
      } else {
        ...
        prefs = {
          ...
          eventMutes: Array.isArray(cp.eventMutes)
            ? cp.eventMutes
            : Array.isArray(tprefs.eventMutes)
              ? tprefs.eventMutes
              : [],
        };
      }
      ...
      const content = buildNotifContent(
        event,
        { ...payload, customerName: payload?.customerName || customerName },
        tenantName,
        audience,
      );
      let anySent = false;
        ...
        if (ch === "email") {
          ...
          if (!smsGate.ok) {
            out.results.push({
              ...
              simulated: false,
              reason:
                smsGate.body?.code || smsGate.body?.error || "sms_metered_out",
            });
          ...
          if (r?.sent) {
            anySent = true;
            writeUsage(meterTenant, "sms", 1).catch(() => {});
          }
        } else if (ch === "push") {
  ...
  async function resolveReceptionistTenant(
    sb: any,
    toRaw: any,
  ): Promise<any | null> {
    ...
    if (digits.length >= 10) {
      ...
      try {
        ...
        for (const t of data || []) {
          const n = String(
            (t.settings as any)?.receptionist?.twilioNumber || "",
          ).replace(/\D/g, "");
  ...
  async function findReceptionistLead(
    sb: any,
    tenantId: string,
    idKey: string,
    last10: string,
    ...
    if (last10) {
      try {
        ...
        if (row) {
          ...
          if (
            RECEPTIONIST_OPEN_STATUSES.has(status) &&
            Date.now() - created < RECEPTIONIST_LEAD_REUSE_MS
          )
            return row;
        }
      } catch {
  ...
  async function ownsCustomerPhone(
    sb: any,
    tenantId: string,
    last10: string,
  ): Promise<boolean> {
  ...
  async function runReceptionistTurn(
    ...
    try {
      ...
      const autoReplyOn =
        recepCfg.enabled !== false && recepCfg.autoReply !== false;
      ...
      if (!msg) {
        ...
        try {
          ...
          console.warn(
            "[receptionist] extract failed; using heuristic:",
            e?.message,
          );
          extracted = normalizeExtraction(extractLeadHeuristic(msg), msg);
        }
      }
      if (callerName && !extracted.name)
        extracted.name = String(callerName).slice(0, 120);
      ...
      const transcriptEntry = msg
        ? [{ from: "caller", text: msg, at: nowISO, channel }]
        : [];
      if (!lead) {
        const data: any = {
      ...
      if (extracted.name && (!data.name || lead.name === "Missed call")) {
        /* name lives on the column */
      }
      ...
      if (rank[extracted.urgency] > rank[data.urgency ?? "low"])
        data.urgency = extracted.urgency;
      if (transcriptEntry.length)
        data.transcript = [
      ...
      if (!data.ownerAlertedAt) {
        out.ownerAlert = await dispatchNotification(
          tenantId,
          null,
          "missed_call",
          {
      ...
      try {
        ...
        if (extracted.name && (lead.name === "Missed call" || !lead.name))
          patch.name = extracted.name;
        if (extracted.address && !lead.address)
          patch.address = extracted.address;
        if (extracted.need && !lead.match_reason)
          patch.match_reason = extracted.need;
  ...
  app.post("/api/agent/receptionist", async (req: any, res) => {
    try {
      ...
      if (!norm.valid)
        return res
          .status(400)
          .json({ error: "A valid caller phone number is required." });
      if (!message || typeof message !== "string" || !message.trim())
        return res.status(400).json({ error: "A caller message is required." });
      const sb = getServiceSupabase();
      const tenant = sb ? await resolveTenant(req) : null;
      if (REQUIRE_AUTH && !tenant)
        return res.status(401).json({ error: "Unauthorized" });
      const ch: any =
        channel === "voice" || channel === "missed_call"
          ? "missed_call"
          : channel === "voicemail"
            ? "voicemail"
            : "inbound_sms";
      if (!tenant) {
        ...
        return res.json({
          ...
          extracted,
          reply: {
            text: buildReceptionistReply({
              name: extracted.name,
              need: extracted.need,
              businessName: "YardWorx",
            }),
            sent: false,
            simulated: true,
          },
      ...
      return res
        .status(500)
        .json({ error: "Could not process the caller message." });
    }
  });
  ...
  app.post("/api/email/send", async (req: any, res) => {
    try {
      ...
      if (!to || !subject)
        return res.status(400).json({ error: "to + subject required" });
 ...
 {"type":"note","risk":"low","title":"..."}
        ...
        return res.json({
          ...
          actions: [
            ...
            },
            {
              ...
              lineItems: [
                { description: "Service", amount: null, fromCatalog: true },
              ],
              total: null,
            },
          ],
  ...
  app.post("/api/measure/property", aiLimiter, async (req: any, res) => {
    try {
      ...
      if (MEASURE_CACHE.has(cacheKey))
        return res.json(MEASURE_CACHE.get(cacheKey));
          ...
          sections: [
            {
              ...
              items: [
                ...
                `Costs ${fmt(m.cost)}`,
                `Overdue AR ${fmt(m.overdueAr)}`,
              ],
            },
            {
              title: "Customers",
              items: [
                `${m.atRisk ?? 0} customers flagged at-risk`,
                `${m.newLeads ?? 0} new leads`,
              ],
            },
            {
              title: "Crew",
              items: [
                `${m.jobsCompleted ?? 0} jobs completed`,
                m.utilizationPct != null
                  ? `${m.utilizationPct}% crew utilization`
                  : "Utilization not tracked",
              ],
            },
          ],
          recommendations: [
            m.overdueAr
              ? "Send payment reminders on overdue invoices."
              : "Keep AR current — send invoices same-day via Closeout.",
            m.atRisk
              ? "Review the at-risk list and trigger a save play."
              : "Ask happy customers for a referral.",
          ],
        });
      }
...
{"headline":"...","summary":"2-3 sentences","sections":[{"title":"Money|Customers|Crew|...","items":["short line", ...]}],"recommendations":["actionable, prioritized", ...]}`;
  ...
  app.post("/api/agent/save-play", aiLimiter, async (req: any, res) => {
    try {
      ...
      if (!play || !play.message)
        return res.status(502).json({ error: "Save-play generation failed" });
      res.json(play);
    } catch (e: any) {
  ...
  const LIVE_MAX_PAYLOAD =
    Number(process.env.LIVE_MAX_PAYLOAD_BYTES) || 1_048_576;
  ...
  wss.on("connection", async (clientWs, req) => {
    ...
    if (liveConnections >= LIVE_CAP) {
      try {
        clientWs.close(1013, "Live capacity reached");
      } catch {}
      return;
    }
    ...
    if (!liveLimiter.reserveIp(ipKey)) {
      try {
        clientWs.close(1013, "Too many Live connections from your network");
      } catch {}
      return;
    }
    ...
    if (REQUIRE_AUTH) {
      let authed = false;
      let overQuota = false;
      let quotaInfo: { limit: number; used: number; tier: string } = {
        limit: 0,
        used: 0,
        tier: "free",
      };
            ...
            try {
              ...
              if (tid) {
                ...
                const { data: t } = await sb
                  ...
                  .eq("id", tid)
                  .maybeSingle();
                liveTenantName = (t?.settings?.businessName || t?.name || "")
                  .toString()
                  .slice(0, 80);
                ...
                if (!gate.ok) {
                  ...
                  quotaInfo = {
                    limit:
                      b.limit ??
                      TIER_ALLOTMENTS[t?.tier || "free"]?.live_min ??
                      0,
                    used: b.used ?? 0,
                    tier: t?.tier || "free",
                  };
      ...
      if (!authed) {
        try {
          clientWs.close(1008, "Session expired — sign in again.");
        } catch {}
        return;
      }
      if (overQuota) {
        ...
        try {
          ...
          clientWs.close(
            4003,
            "You've used your Live-Ear minutes for this month. Upgrade or raise your spend cap to keep going.",
          );
        } catch {}
        return;
      }
    ...
    if (REQUIRE_AUTH && liveMeterTenant?.id) {
      ...
      if (!liveLimiter.reserveTenant(tenantKey)) {
        try {
          clientWs.close(
            1013,
            "Too many simultaneous Live sessions for your account",
          );
        } catch {}
        return;
      }
    ...
    clientWs.on("close", () => {
      ...
      if (liveMeterTenant && liveStartedAt) {
        const minutes = Math.max(
          1,
          Math.ceil((Date.now() - liveStartedAt) / 60000),
        );
    ...
    if (isMockMode) {
      ...
      const demo: any[] = [
        ...
        {
          transcription:
            'Heard: "Let\'s redo the front bed with some hydrangeas."',
        },
        {
          demo: true,
          action: {
            functionCalls: [
              {
                ...
                args: { clientName: "current customer" },
              },
            ],
          },
        },
        {
          transcription:
            "Pulling up the customer and drafting a design vision…",
        },
        {
          demo: true,
          action: {
            functionCalls: [
              {
                ...
                args: { service: "Planting bed install" },
              },
            ],
          },
        },
      ...
      const timer = setInterval(() => {
        if (clientWs.readyState !== 1 || i >= demo.length) {
          clearInterval(timer);
          return;
        }
      ...
      clientWs.on("message", () => {
        /* ignore client audio/video in mock mode */
      });
    ...
    try {
      const session = await ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (e: any) => {
            ...
            sessionClosed = true;
            try {
              clientWs.close(1011, "Live session error");
            } catch {}
          },
          onclose: (e: any) => {
            ...
            sessionClosed = true;
            try {
              clientWs.close(1000, "Live session ended");
            } catch {}
          },
              ...
              if (toolCall.functionCalls) {
                session.sendToolResponse({
                  functionResponses: toolCall.functionCalls.map((fc) => ({
                    ...
                    response: {
                      result:
                        "Action queued for dispatch in the operations UI.",
                    },
                  })),
                });
              }
              ...
              functionDeclarations: [
                {
                  name: "build_design_vision",
                  description:
                    "Open the Design Studio to build a live design vision (photo + AI render + tiered quote) for the customer being discussed. Use when the rep talks about redesigning, planting, hardscaping, or showing the customer ideas.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      clientName: {
                        type: Type.STRING,
                        description: "The customer this vision is for",
                      },
                      focus: {
                        type: Type.STRING,
                        description:
                          "What to redesign, e.g. 'front foundation bed'",
                      },
                    },
                  },
                },
                ...
                },
                {
                  name: "log_expense",
                  description:
                    "Log an expense for a material purchase, fuel, or supply. Use this when the user shows a receipt or mentions spending money.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      amount: {
                      ...
                      category: {
                        type: Type.STRING,
                        description:
                          "The category (e.g. Fuel, Supplies, Unknown)",
                      },
                      merchant: {
                        type: Type.STRING,
                  ...
                  parameters: {
                    ...
                    properties: {
                      ...
                      quietHoursStart: {
                        type: Type.STRING,
                        description:
                          "Optional earliest service time, e.g. '09:00'.",
                      },
                    },
      ...
      log.error("Gemini Live connection failed", error, {
...
if (process.env.VITEST) {
  ...
  console.log(
    `Setting up ${numCPUs} worker(s) (cap=${cap}, host cores=${os.cpus().length})...`,
  );
  ...
  const shutdownPrimary = (signal: string) => {
    ...
    log.info("primary shutdown: forwarding signal to workers", {
      signal,
      pid: process.pid,
    });
    for (const w of Object.values(cluster.workers || {})) {
      try {
        (w as any)?.kill(signal);
      } catch {}
    }
    const t = setTimeout(
      () => process.exit(0),
  ...
  cluster.on("exit", (worker, code, signal) => {
    ...
    console.log(
      `Worker ${worker.process.pid} died (code=${code}, signal=${signal}). Respawning in 1s...`,
    );
