...
export async function createApp({ startListening = false } = {}) {
  ...
  app.use((req, res, next) => {
    ...
    // Path/command patterns are URL-only (a note saying "walk ../ back" shouldn't 403), except for
    // /api/translate where parameter path traversal and DAX payloads in request body must be blocked (403).
