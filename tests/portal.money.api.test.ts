...
describe('portal money endpoints (mock mode: Supabase faked, Stripe unset)', () => {
  ...
  describe('portal-token gate', () => {
    ...
    it('401s on a token signed with an unsupported algorithm (e.g. HS384 algorithm confusion defense)', async () => {
      const wrongAlgo = jwt.sign({ clientId: CLIENT, scope: 'portal' }, JWT_SECRET, { algorithm: 'HS384', expiresIn: '1h' });
      const res = await request(app)
        .post('/api/portal/checkout')
