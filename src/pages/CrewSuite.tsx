...
import JobTimeClock from "../components/JobTimeClock";
...
export default function CrewSuite() {
  ...
  // The clock UI lives in the reusable <JobTimeClock> control below; it stamps
  // jobId + customerId, does optional geofenced clock-in, and is offline-safe. Here we
  // just load the active/scheduled jobs it offers in its picker.
  ...
  return (
    <SubscriptionGuard requiredTier="enterprise">
      ...
      {/* TIME CLOCK — per-job time entries for accurate Job Costing. The reusable control
          stamps jobId + customerId, offers optional geofenced clock-in, shows the live
          open shift + elapsed, and is offline-safe (syncService queue). Hours feed the
          shipped payroll export (Reports → Payroll). */}
      <div className="mt-10 max-w-md">
        <JobTimeClock jobs={clockJobs} onToast={showToast} />
      </div>
