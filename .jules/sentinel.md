...
## 2025-06-20 - SSRF Bypass via Hex-Encoded IPv4-Mapped IPv6
**Vulnerability:** SSRF filter bypass using hex-encoded IPv4-mapped IPv6 addresses (e.g., ::ffff:7f00:1).
**Learning:** The 'isPrivateIP' utility only handled dotted-decimal notation for mapped addresses. Attackers can bypass such filters by using hex notation which many libraries or OS layers resolve to the same internal IP.
**Prevention:** Always normalize the suffix of IPv4-mapped IPv6 addresses (handling both dotted-decimal and hex) before performing range checks.
