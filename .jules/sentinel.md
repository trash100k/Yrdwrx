...
## 2026-07-09 - Hex-Encoded IPv4-mapped IPv6 SSRF Bypass
**Vulnerability:** Server-Side Request Forgery (SSRF) filters could be bypassed using hex-encoded IPv4-mapped IPv6 addresses (e.g., `::ffff:7f00:1`).
**Learning:** Standard IP validation libraries (like `net.isIP` in Node.js) and simple regex for `::ffff:d.d.d.d` do not catch hex-encoded variations of IPv4-mapped IPv6 addresses. These addresses are valid and resolved by many network stacks to local/private IPv4 addresses.
**Prevention:** SSRF protection utilities must normalize or explicitly handle hex-encoded IPv4-mapped IPv6 addresses by parsing the 32-bit hex tail into dotted-decimal IPv4 and re-validating against private ranges. Also, explicitly block the unspecified address `::`.
